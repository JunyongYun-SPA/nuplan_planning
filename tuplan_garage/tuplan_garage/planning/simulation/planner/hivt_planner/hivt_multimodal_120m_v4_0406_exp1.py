...
class HivtMultimodal(AbstractPDMPlanner):
    ...
    def compute_planner_trajectory(
        ...
        try:
            ...
            for route_count_th in route_count_ths:
                ...
                for mode_, single_traj in enumerate(y_hat):
                    traj_in_route = points_in_polygons(single_traj.detach().numpy(), temporal_feature["local_route"])
                    all_lane_in_route = []
                    ...
                    if len(np.where(all_lane_in_route==True)[0]) > route_count_th:
                        y_hat_in_route_mask_mode_index.append(mode_)
                if len(y_hat_in_route_mask_mode_index) >= 3:
                    break
            if len(y_hat_in_route_mask_mode_index) == 0:
                raise ValueError
            y_hat_ = y_hat[y_hat_in_route_mask_mode_index, ...]
        ...
        # pi 중 가장 좋은 것
        result_pi_index = torch.argmax(pi_)
        y_hat__ = y_hat_[result_pi_index].unsqueeze(0)
        
        current_ego_vel = np.array([current_input.history.current_state[0].dynamic_car_state.speed])
        traj_w_heading = waypoints_to_trajectory(y_hat__, current_ego_vel).detach().numpy()[0]
