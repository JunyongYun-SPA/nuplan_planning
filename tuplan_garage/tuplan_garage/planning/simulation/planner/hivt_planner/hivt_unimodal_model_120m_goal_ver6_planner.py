...
#     create_pdm_feature,
# )
from tuplan_garage.planning.simulation.planner.hivt_planner.utils.hivt_120m_goal_feature_utils_ver4 import (
    create_hivt_feature,
)
...
class HivtUnimodal(AbstractPDMPlanner):
    ...
    def compute_planner_trajectory(
        ...
        pdm_feature, vis_pack = create_hivt_feature(self._model, current_input, self._centerline, None, self._model.device, scenario=scenario, map_api=self._map_api, initialization = self.initialization)
        pdm_feature = pdm_feature.copy()
        temporal_feature = TemporalData(**pdm_feature)
        # torch.save(temporal_feature, cache_path)
        self._model.device = 'cpu'
        ...
        # traj_w_heading = predictions["trajectory"][:,temporal_feature['av_index'],:, :3].squeeze().detach().numpy()
        # XY 예측이 괜찮다는 사실을 증명해야 함
        ...
        # traj_w_heading = temporal_feature.y[temporal_feature.av_index]
