...
def get_lane_features(current_input, map_api, node_inds, node_positions, origin, rot_mat, radius, end_lanes, av_ind):
    ...
    for node_ind, node_position in zip(node_inds, node_positions):
        ...
        if node_ind == av_ind:
            ...
            LANE_obj = map_api.get_proximal_map_objects(position, 50, LANE_LAYER)
...
def get_lane_features(current_input, map_api, node_inds, node_positions, origin, rot_mat, radius, end_lanes, av_index):
...
def create_hivt_feature(
    ...
    (lane_vectors, is_intersections, turn_directions, traffic_controls, lane_actor_index,
        lane_actor_vectors, lane_positions, goals, in_route_lanes) = get_lane_features(planner_input, initialization.map_api, node_inds, node_positions, origin, rotate_mat, radius, end_lanes, av_index=actor_ids.index('ego'))
