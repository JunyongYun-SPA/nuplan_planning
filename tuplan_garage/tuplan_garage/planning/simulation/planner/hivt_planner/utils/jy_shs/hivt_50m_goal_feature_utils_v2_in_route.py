...
def create_hivt_feature(
    ...
    centerline_progress_values = (
        np.arange(120, dtype=np.float64) # Test centerline
        ...
        if len(indexs) != 0:
            for index in indexs:
                ...
                if road_block_id_from_Lane in initialization.route_roadblock_ids:
                    ...
                    cliped_polygon.append(map_api._get_roadblock(road_block_id_from_Lane).polygon)
                    if find_goal == False:
        ...
        if len(indexs) != 0:
            for index in indexs:
                ...
                if road_block_id_from_con in initialization.route_roadblock_ids:
                    ...
                    cliped_polygon.append(map_api._get_roadblock_connector(road_block_id_from_con).polygon)
                    if find_goal == False:
    ...
    # Get AV_centriced cliped route roadblock 
    local_route_polygon = []
    ...
    (lane_vectors, is_intersections, turn_directions, traffic_controls, lane_actor_index,
        lane_actor_vectors, lane_positions, goals) = get_lane_features(planner_input, initialization.map_api, node_inds, node_positions, origin, rotate_mat, radius, end_lanes)
    ...
    results = {'x':x, # x.shape ~ [15, 27, 3]
            ...
            'theta':theta,
            'goal': goals,
            'AV_centric_route': local_route_polygon,
