...
#     create_pdm_feature,
# )
from tuplan_garage.planning.simulation.planner.hivt_planner.utils.hivt_120m_goal_feature_utils_ver7 import (
    create_hivt_feature,
)
...
class HivtMultimodal(AbstractPDMPlanner):
    ...
    def compute_planner_trajectory(
        ...
        modal_num = predictions['pi'].shape[1]
        
        pi = predictions['pi']
        pi = pi[pdm_feature['av_index'], :]
        pi_best_mode = torch.argmax(pi, dim=-1)
        predictions['trajectory'] = predictions["trajectory"][pi_best_mode, torch.arange(predictions["trajectory"].shape[1])]
        
        # other_agent_index_list = [i for i in range(pdm_feature['num_nodes']) if i != pdm_feature['av_index']]
        
        # av_ade = torch.norm(predictions['trajectory'][pdm_feature['av_index'], :, :2] - temporal_feature.y[pdm_feature['av_index'], :, :], p=2, dim=-1).mean(dim=-1)
        # av_fde = torch.norm(predictions['trajectory'][pdm_feature['av_index'], -1, :2] - temporal_feature.y[pdm_feature['av_index'], -1, :], p=2, dim=-1)
        
        # reg_mask = ~temporal_feature['padding_mask'][:, 11:]
        # reg_mask = reg_mask[other_agent_index_list]
        # valid_steps = (reg_mask.sum(dim=-1) == 16)
        
        # other_agent_ade = torch.norm(predictions['trajectory'][other_agent_index_list, :, :2][valid_steps] - temporal_feature.y[other_agent_index_list, :, :][valid_steps], p=2, dim=-1).mean(dim=-1).mean()
        # other_agent_fde = torch.norm(predictions['trajectory'][other_agent_index_list, -1, :2][valid_steps] - temporal_feature.y[other_agent_index_list, -1, :][valid_steps], p=2, dim=-1).mean()
        
        # error_list = [av_ade, av_fde, other_agent_ade, other_agent_fde]
        # with open(f"/home/workspace/ade_fde/hivt_multimodal_120m_v4_goal_planner_ver3_modal{modal_num}/{scenario.token}_{self._iteration}.pkl","wb") as f:
