...
#     create_pdm_feature,
# )
from tuplan_garage.planning.simulation.planner.hivt_planner.utils.hivt_50m_goal_feature_utils import (
    create_hivt_feature,
)
...
class HivtUnimodal(AbstractPDMPlanner):
    ...
    def _vis(self, temporal_feature, vis_pack, predictions, img_path):
        ...
        pos_xs = predictions['trajectory'][...,:2].numpy()[0, :, 0]
        pos_ys = predictions['trajectory'][...,:2].numpy()[0, :, 1]
        plt.scatter(pos_xs, pos_ys, s = 30, c="limegreen", edgecolor='black', alpha=1.0)
                    
                
        # lane_x = lane_positions[:, 0]
        # lane_y = lane_positions[:, 1]
        # plt.scatter(lane_x, lane_y, s = 50, c="black", alpha=0.8)
    ...
    def compute_planner_trajectory(
        ...
        # TODO list: multi modal 추가해야함
        # y_hat, pi = predictions['trajectory'], predictions['pi']
        # pi = pi[pdm_feature['av_index']][:]
        # pi_best_mode = torch.max(pi)
        # pi_best_mode = pi_best_mode.indices
