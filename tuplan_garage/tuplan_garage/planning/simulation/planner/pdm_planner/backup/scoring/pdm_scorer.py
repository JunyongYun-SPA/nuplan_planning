...
class PDMScorer:
    ...
    def score_proposals(
        ...
        self._calculate_ego_area_only_center()

        # 1. multiplicative metrics
        self._calculate_no_at_fault_collision_bh_Ver() # 0
        self._calculate_driving_direction_compliance() #1 
        self._calculate_drivable_area_compliance() #2

        # 2. weighted metrics
        self._calculate_progress() #3
        self._calculate_ttc() # 속도 등의 요인이 들어가기 때문임 # 4
        self._calculate_is_comfortable() # 속도 각 가속도, 각속도 etc.. #5 
    ...
    def _aggregate_scores(self) -> npt.NDArray[np.float64]:
        ...
        elem = [0,1,2,3,4,5]
        for i in range(1, 7):
            for item in combinations(elem, i):
                all_combi.append(np.array(list(item)))
        # 바로 사용할 수 있는 score metric 구축
        instance_metric = np.zeros((6, 6))
        instance_metric[:3, :] = self._multi_metrics.copy()
        instance_metric[3, :] = self._weighted_metrics[0].copy() * WEIGHTED_METRICS_WEIGHTS[0]/WEIGHTED_METRICS_WEIGHTS.sum()
        instance_metric[4, :] = self._weighted_metrics[1].copy() * WEIGHTED_METRICS_WEIGHTS[1]/WEIGHTED_METRICS_WEIGHTS.sum()
        instance_metric[5, :] = self._weighted_metrics[2].copy() * WEIGHTED_METRICS_WEIGHTS[2]/WEIGHTED_METRICS_WEIGHTS.sum()
        combi_test = {}
        for combi_ in all_combi:
            sum_index = combi_[np.where(combi_>=3)[0]]
            if len(sum_index) != 0:
                tmp_result = instance_metric[sum_index, :].sum(axis=0)
            else:
                tmp_result = np.ones((1, 6))
            mul_index = combi_[np.where(combi_<=2)[0]]
            if len(mul_index) != 0:
                tmp_result = instance_metric[mul_index, :].prod(0) * tmp_result
            combi_test[str(combi_)] = tmp_result.copy()
    ...
    def _calculate_ego_area_original(self) -> None:
        """
        Determines the area of proposals over time.
        Areas are (1) in multiple lanes, (2) non-drivable area, or (3) oncoming traffic
        """
        
        n_proposals, n_horizon, n_points, _ = self._ego_coords.shape # 6, 9, 5, 2 # BH(기준 완화를 위해 center로 바꾸는게 좋을듯) ~ self._ego_coords[0][0][4]가 center point
        coordinates = self._ego_coords.reshape(n_proposals * n_horizon * n_points, 2) #(270, 2)
        ...
        center_in_polygon_ = np.expand_dims(center_in_polygon, axis=-1)
    ...
    def _calculate_ego_area_only_center(self) -> None:
        """
        Determines the area of proposals over time.
        Areas are (1) in multiple lanes, (2) non-drivable area, or (3) oncoming traffic
        # 1. drivable area 여부는 centerline만 확인하기
        # 2. direction과 관련있는 oncoming traffic은 .. 음
        """
        ...
        # in_nondrivable_area: if at least one corner is not within any drivable polygon
        batch_nondrivable_area_mask = np.zeros((n_proposals, n_horizon), dtype=np.bool_)
        batch_nondrivable_area_mask = (center_in_polygon_.sum(axis=-2) > 0).sum(
            axis=-1
        ) < 1
        self._ego_areas[
            batch_nondrivable_area_mask, EgoAreaIndex.NON_DRIVABLE_AREA
    ...
    def _calculate_no_at_fault_collision_original(self) -> None:
        """
    ...
    def _calculate_no_at_fault_collision_bh_Ver(self) -> None:
        """
        Re-implementation of nuPlan's at-fault collision metric.
        1. Hz를 증가시켜 세밀하게 충돌 감지?
        2. 음..
        """
        no_collision_scores = np.ones(self._num_proposals, dtype=np.float64)
    ...
    def _calculate_ttc(self):
        ...
        future_time_idcs = np.arange(0, 3, 1) # BH 수정 10이었던걸 2로 기존 ~[0, 3, 6, 9] 이후 [0, 1, 2]
        ...
        # for time_idx in range(self._proposal_sampling.num_poses + 1): # num_pose = 16
        for time_idx in range(16): # 속도가 유효하지 않은 마지막 포인트 제외
    ...
    def _calculate_is_comfortable(self) -> None:
        ...
        is_comfortable = ego_is_comfortable(self._states[:, :-2, :], time_point_s)
    ...
    def _calculate_driving_direction_compliance(self) -> None:
        ...
        for proposal_idx in range(self._num_proposals):
            ...
            # if sum(oncoming_traffic_progress) == 0:
            #     raise ValueError
