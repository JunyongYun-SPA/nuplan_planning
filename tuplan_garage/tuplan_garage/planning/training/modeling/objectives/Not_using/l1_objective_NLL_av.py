...
class L1Objective(AbstractObjective):
    ...
    def __init__(
        ...
        self._weight = weight
        self._loss_function = LaplaceNLLLoss(reduction='mean')
    ...
    def compute(
        ...
        """
        predicted_trajectory = predictions["trajectory"][targets['pdm_features']['av_index'], :, :4]
        targets_trajectory = targets['pdm_features']['y'][targets['pdm_features']['av_index'], :16, :2]
        ...
        loss = self._loss_function(
            predicted_trajectory.reshape(-1, 4),
            targets_trajectory.reshape(-1, 2),
        )
        if prefix == 'train':
