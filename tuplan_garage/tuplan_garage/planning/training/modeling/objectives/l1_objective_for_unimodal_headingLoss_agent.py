...
class L1ObjectiveAgent(AbstractObjective):
    """
    ...
    def __init__(
        ...
        """
        self._name = "l1_objective_agent"
        self._weight = weight
        self._loss_function_traj = torch.nn.modules.loss.L1Loss(reduction="none")
    ...
    def compute(
        ...
        loss_traj = self._loss_function_traj(
            predicted_trajectory[reg_mask][:, :2].reshape(predicted_trajectory[reg_mask].shape[0], -1),
            targets_trajectory[reg_mask][:, :2].reshape(predicted_trajectory[reg_mask].shape[0], -1),
        )
        loss_head = -torch.cos((predicted_trajectory[reg_mask][:, 2].unsqueeze(-1) - targets_trajectory[reg_mask][:, 2].unsqueeze(-1)))
