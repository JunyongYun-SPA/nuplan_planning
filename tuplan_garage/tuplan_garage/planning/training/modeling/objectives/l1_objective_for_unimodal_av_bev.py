...
class L1ObjectiveAV(AbstractObjective):
    ...
    def compute(
        ...
        loss_first_layer = self._loss_function(
            predicted_trajectory[0][reg_mask].reshape(predicted_trajectory[0][reg_mask].shape[0], -1),
            targets_trajectory[reg_mask].reshape(predicted_trajectory[0][reg_mask].shape[0], -1),
        )
        loss_second_layer = self._loss_function(
            predicted_trajectory[1][reg_mask].reshape(predicted_trajectory[1][reg_mask].shape[0], -1),
            targets_trajectory[reg_mask].reshape(predicted_trajectory[1][reg_mask].shape[0], -1),
        )
        loss_last_layer = self._loss_function(
            predicted_trajectory[-1][reg_mask].reshape(predicted_trajectory[-1][reg_mask].shape[0], -1),
            targets_trajectory[reg_mask].reshape(predicted_trajectory[-1][reg_mask].shape[0], -1),
        )
        loss = torch.mean(loss_first_layer) + torch.mean(loss_second_layer) + torch.mean(loss_last_layer)
        # loss = torch.mean(loss_last_layer)
