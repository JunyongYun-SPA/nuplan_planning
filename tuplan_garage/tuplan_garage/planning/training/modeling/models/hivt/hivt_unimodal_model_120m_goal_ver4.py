...
from tuplan_garage.planning.training.preprocessing.feature_builders.hivt_feature_builder_120m_goal_ver12 import (
    HiVTFeatureBuilder,
)
...
from tuplan_garage.planning.training.modeling.models.hivt.local_encoder_120m_goal_ver4 import LocalEncoder
...
class HiVTModel(TorchModuleWrapper): #JY
    ...
    def __init__(
        ...
        self.global_interactor = GlobalInteractor(historical_steps=11,
                                                  ...
                                                  dropout=0.1,
                                                  rotate=True)
        self.decoder = MLPDecoder(local_channels=32, # 64
                                  global_channels=32, # 64
                                  future_steps=16,
                                  num_modes=self.num_modes,
                                  uncertain=True)
        ...
        # bh
        self.encoder_consume_time = []
        self.gl_encoder_consume_time = []
        self.decoder_consume_time = []
        self.model_consume_time = []
    ...
    def forward(self, features: FeaturesType) -> TargetsType:
        ...
        edge_index_ = input.edge_index.detach().cpu()
        edge_index_unique_node = torch.unique(edge_index_[0,:])
        x_ = input.x.detach().cpu().numpy()
        x_range = torch.arange(len(x_))
        if edge_index_unique_node.shape != x_range.shape:
            print()
        ...
        # edge_index_size = input['edge_index'].shape[-1]
        # lane_actor_index_size = input['lane_actor_index'].shape[-1]
        # print(f"SIZE edge_index/lane_actor_index => {edge_index_size} /{lane_actor_index_size}")
        consume_time = time.time()-cur_t
        self.model_consume_time.append(consume_time)
