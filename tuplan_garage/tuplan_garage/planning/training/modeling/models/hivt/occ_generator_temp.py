...
class OccGenerator(nn.Module):
    ...
    def __init__(self,
        ...
        self.transformer = DeformableTransformer(d_model=embed_dim, nhead=8,
                 num_encoder_layers=6, num_decoder_layers=6, dim_feedforward=128, dropout=0.1,
                 activation="relu", return_intermediate_dec=False,
                 num_feature_levels=4, dec_n_points=4,  enc_n_points=4,
                 two_stage=False)
        
        self.occ_query = nn.Parameter(torch.Tensor(4, 200, 200, 64))
    ...
    def forward(self, data: TemporalData, occupancy_map) -> torch.Tensor:
        ...
        hs, init_reference, inter_references, _, _= self.transformer(occ_query_u1, occupancy_map)
        ...
        out = self.occupancy_decoer(hs).reshape(-1, 200, 200, 16).permute(0, 3, 1, 2)
...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, query_embed, memory):
        ...
        hs, inter_references = self.decoder(tgt, reference_points, memory,
                                            torch.tensor([[200, 200]], device=query_embed.device), torch.tensor([0], device=query_embed.device), query_embed)

        inter_references_out = inter_references
        # if self.two_stage:
        #     return hs, init_reference_out, inter_references_out, N, enc_outputs_coord_unact
...
def OccupancyGeneratorParallelOnlyFuture(input, y_hat, pi):
    ...
    occupancy_map = occupancy_map_future
...
def OccupancyGeneratorParallelforSimulation(input, y_hat, pi):
    ...
    occupancy_size = 200
    occupancy_range = int(occupancy_size * occupancy_resolution)
    occupancy_map_env = torch.zeros(1, 1, occupancy_size, occupancy_size, dtype=torch.float32).to(pi.device)
    occupancy_map_history = torch.zeros(1, 11, occupancy_size, occupancy_size, dtype=torch.float32).to(pi.device)
    occupancy_map_future = torch.zeros(1, 16, occupancy_size, occupancy_size, dtype=torch.float32).to(pi.device)
    occupancy_map_gt = torch.zeros(1, 16, occupancy_size, occupancy_size, dtype=torch.float32).to(pi.device)
    drop_edge_av = DistanceDropEdge(occupancy_range/2)
    input['edge_attr'] = \
            input['positions'][input['edge_index'][0], 10, :2] - input['positions'][input['edge_index'][1], 10, :2]
    edge_index, _ = drop_edge_av(input['edge_index'], input['edge_attr'])
    lane_edge_index, lane_attr = drop_edge_av(input['lane_actor_index'], input['lane_actor_vectors'])
    ...
    # batch_idx_for_agent = torch.zeros(others_indx.shape[0], dtype=torch.int64) #others_indx.unsqueeze(-1).repeat(1, 16) - input.ptr.unsqueeze(0)[:, 1:]
    batch_idx_for_agent = torch.zeros(others_indx.shape[0], dtype=torch.int64) #torch.where((batch_idx_for_agent > 0), 1, 0).sum(dim=-1)
    
    # batch_idx_for_lane = lane_edge_index[1].unsqueeze(-1).repeat(1, 16) - input.ptr.unsqueeze(0)[:, 1:]
    batch_idx_for_lane = torch.zeros(lane_edge_index[1].shape[0], dtype=torch.int64) #torch.where((batch_idx_for_lane > 0), 1, 0).sum(dim=-1)
    occ_mask_lane = (abs(lane_attr[:, 0]) < 50) * (abs(lane_attr[:, 1]) < 50)
    lane = torch.where(((occ_mask_lane).unsqueeze(-1)),
                            lane_attr,
                            torch.zeros(lane_attr.shape[0], 2).to(lane_attr.device))
    occupancy_x_lane, occupancy_y_lane = \
                (lane / occupancy_resolution).type(torch.int)[:, 0], \
                    (lane / occupancy_resolution).type(torch.int)[:, 1]
    occupancy_map_env[batch_idx_for_lane, \
    ...
    # agent_pred_pi = F.softmax(pi[others_indx], dim=-1).unsqueeze(-1).repeat(1, 1, 16).permute(1, 0, 2).reshape(-1)
    ...
    agent_future = torch.where((((~input.padding_mask[others_indx, 11:]) * occ_mask_future).unsqueeze(-1)),
                            agent_future,
                            torch.zeros(others_indx.shape[0], 16, 2).to(agent_future.device))
    agent_pred = torch.where((((~input.padding_mask[others_indx, 11:]).unsqueeze(0) * occ_mask_pred).unsqueeze(-1)),
                            agent_pred,
                            torch.zeros(y_hat.shape[0], others_indx.shape[0], 16, 2).to(agent_pred.device))
    ...
    occupancy_x_pred, occupancy_y_pred = \
                (agent_pred / occupancy_resolution).type(torch.int)[:, :, :, 0], \
                    (agent_pred / occupancy_resolution).type(torch.int)[:, :, :, 1]
    a = batch_idx_for_agent.unsqueeze(0).unsqueeze(-1).repeat(occupancy_x_pred.shape[0], 1, 16).reshape(-1)
    b = torch.arange(16).unsqueeze(0).unsqueeze(1).repeat(occupancy_x_pred.shape[0], occupancy_x_pred.shape[1], 1).reshape(-1)
    c = (occupancy_range - occupancy_y_pred).type(torch.long).reshape(-1)
    d = (occupancy_x_pred + occupancy_range).type(torch.long).reshape(-1)
    # pi_dict = {(int(a[idx]), int(b[idx]), int(c[idx]), int(d[idx])): [] for idx, _ in enumerate(range(a.shape[0]))}
    # for i, j, k, l, p in zip(a, b, c, d, agent_pred_pi):
    #     pi_dict[(int(i), int(j), int(k), int(l))].append(float(p))
    # for key, value in pi_dict.items():
    #     i, j, k, l = key
    #     occupancy_map_future[i, j, k, l] = max(value)
    #     # max_pi = [max(i) for i in pi_dict.values()]
    
    occupancy_map_future[a, b, c, d] = 1
    
    occupancy_map_history[:, :, 100, 100] = 0
    occupancy_map_future[:, :, 100, 100] = 0
    occupancy_map_gt[:, :, 100, 100] = 0
    occupancy_map_env[:, :, 100, 100] = 0
    
    occupancy_map = torch.cat((occupancy_map_env, occupancy_map_history, occupancy_map_future), dim=1)
    
    return occupancy_map, occupancy_map_gt

def OccupancyGeneratorParallelUnimodal(input, y_hat, pi):
    modal_shape, agent_shape, future_time_shape, coord_shape = y_hat.shape
    batch_shape = input.av_index.shape[0]
