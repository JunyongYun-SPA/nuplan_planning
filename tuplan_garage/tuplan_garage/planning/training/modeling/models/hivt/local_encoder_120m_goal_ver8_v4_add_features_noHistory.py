...
class LocalEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.drop_edge_agent = DistanceDropEdgeOtherAgents(local_radius)
        ...
        self.aa_encoder_av = AAEncoderExtraFeatures(historical_steps=historical_steps,
                                    node_dim=node_dim,
                                    edge_dim=edge_dim,
                                    embed_dim=embed_dim,
                                    num_heads=num_heads,
                                    dropout=dropout,
                                    parallel=parallel)
        self.aa_encoder_agent = AAEncoderExtraFeatures(historical_steps=historical_steps,
                                    node_dim=node_dim,
                                    edge_dim=edge_dim,
                                    embed_dim=embed_dim,
                                    num_heads=num_heads,
    ...
    def forward(self, data: TemporalData) -> torch.Tensor:
        ...
        if self.parallel:
            ...
            for t in range(self.historical_steps):
                edge_index, edge_attr = self.drop_edge_agent(data[f'edge_index_{t}'], data[f'edge_attr_{t}'])
            ...
            for t in range(self.historical_steps):
                edge_index, edge_attr = self.drop_edge_agent(data[f'edge_index_{t}'], data[f'edge_attr_{t}'], data['av_index'])
                out[t] = self.aa_encoder_agent(x=data.x[~av_mask, t, :], t=t, edge_index=edge_index, edge_attr=edge_attr,
                                         bos_mask=data['bos_mask'][~av_mask, t], rotate_mat=data['rotate_mat'][~av_mask], rotate_angles=data['rotate_angles'][~av_mask])
