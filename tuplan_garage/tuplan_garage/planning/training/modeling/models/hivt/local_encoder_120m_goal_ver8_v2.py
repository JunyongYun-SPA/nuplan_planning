...
class LocalEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.al_encoder = ALEncoder(node_dim=node_dim,
                                    ...
                                    num_heads=num_heads,
                                    dropout=dropout)
        # self.al_encoder2 = ALEncoder(node_dim=node_dim,
        #                             edge_dim=edge_dim,
        #                             embed_dim=embed_dim,
        #                             num_heads=num_heads,
        #                             dropout=dropout)
    ...
    def forward(self, data: TemporalData) -> torch.Tensor:
        for t in range(self.historical_steps):
            try:
                ...
                data[f'edge_index_{t}'] = torch.zeros(2, 1, dtype=torch.int64)
            data[f'edge_attr_{t}'] = \
        ...
        out_agent = self.al_encoder(x=(data['lane_vectors'], out), edge_index=edge_index, edge_attr=edge_attr,
                              is_intersections=data['is_intersections'], turn_directions=data['turn_directions'],
                              traffic_controls=data['traffic_controls'], rotate_mat=data['rotate_mat'])
