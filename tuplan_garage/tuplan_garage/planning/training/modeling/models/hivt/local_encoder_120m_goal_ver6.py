...
from tuplan_garage.planning.training.modeling.models.hivt.hivt_utils import GetInRouteLaneEdge
...
class LocalEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.drop_edge_al = GetInRouteLaneEdge(local_radius)
        ...
        self.al_encoder = ALEncoderGoal(node_dim=2,
                                    edge_dim=edge_dim,
                                    embed_dim=embed_dim,
                                    num_heads=num_heads,
                                    dropout=dropout)
    ...
    def forward(self, data: TemporalData) -> torch.Tensor:
        ...
        if self.parallel:
            ...
            out = self.aa_encoder(x=batch.x[:, :], t=None, edge_index=batch.edge_index, edge_attr=batch.edge_attr,
                                  bos_mask=data['bos_mask'], rotate_mat=data['rotate_mat'], angles = data['rotate_angles'])
            ...
            for t in range(self.historical_steps):
                ...
                out[t] = self.aa_encoder(x=data.x[:, t, :], t=t, edge_index=edge_index, edge_attr=edge_attr,
                                         bos_mask=data['bos_mask'][:, t], rotate_mat=data['rotate_mat'], angles = data['rotate_angles'])
...
class AAEncoder(MessagePassing):
    ...
    def forward(self,
        ...
        if self.parallel:
            ...
            if rotate_mat is None:
                ...
                angle_embedding[:, 0] = torch.sin(x[:, -1])
                angle_embedding[:, 1] = torch.cos(x[:, -1])
                center_embed = self.center_embed(torch.cat((x_, angle_embedding), dim=-1))
    ...
    def message(self,
        ...
        if rotate_mat is None:
            ...
            x_j_ = torch.bmm(x_j.unsqueeze(-2)[...,:2], center_rotate_mat).squeeze()
            nbr_embed = self.nbr_embed([x_j_.squeeze(-2),
