...
class MLPDecoder(nn.Module):
    ...
    def forward(self,
        ...
        loc = self.loc(out).view(self.num_modes, -1, self.future_steps, 3)  # [F, N, H, 2]
        if self.uncertain:
...
class MLPDecoderIntermediate(nn.Module):
    ...
    def __init__(self,
                 ...
                 uncertain: bool = True,
                 min_scale: float = 1e-3) -> None:
        super(MLPDecoderIntermediate, self).__init__()
        self.input_size = global_channels
        self.hidden_size = local_channels
        self.future_steps = future_steps
        self.num_modes = num_modes
    ...
    def forward(self,
        ...
        if self.uncertain:
            ...
            return loc, pi, out  # [F, N, H, 2], [N, F]
        
class MLPDecoderRef(nn.Module):
    ...
    def __init__(self,
                 ...
                 uncertain: bool = True,
                 min_scale: float = 1e-3) -> None:
        super(MLPDecoderRef, self).__init__()
        self.input_size = global_channels
        self.hidden_size = local_channels
        self.future_steps = future_steps
        self.num_modes = num_modes
    ...
    def forward(self,
        ...
        out = self.aggr_embed(global_embed)
        loc = self.loc(out).view(self.num_modes, -1, self.future_steps, 3)  # [F, N, H, 2]
        if self.uncertain:
            scale = F.elu_(self.scale(out), alpha=1.0).view(self.num_modes, -1, self.future_steps, 3) + 1.0
            scale = scale + self.min_scale  # [F, N, H, 2]
...
class MLPDecoderRefV2(nn.Module):
    ...
    def __init__(self,
                 ...
                 uncertain: bool = True,
                 min_scale: float = 1e-3) -> None:
        super(MLPDecoderRefV2, self).__init__()
        self.input_size = global_channels
        self.hidden_size = local_channels
        self.future_steps = future_steps
        self.num_modes = num_modes
    ...
    def forward(self,
        ...
        pi = self.pi(global_embed).squeeze(-1).t()
        out = self.aggr_embed(global_embed)
        loc = self.loc(out).view(-1, self.future_steps, 3)  # [F, N, H, 2]
        if self.uncertain:
            scale = F.elu_(self.scale(out), alpha=1.0).view(-1, self.future_steps, 3) + 1.0
            scale = scale + self.min_scale  # [F, N, H, 2]
            return torch.cat((loc, scale), dim=-1), pi  # [F, N, H, 4], [N, F]
        else:
            return loc, pi  # [F, N, H, 2], [N, F]