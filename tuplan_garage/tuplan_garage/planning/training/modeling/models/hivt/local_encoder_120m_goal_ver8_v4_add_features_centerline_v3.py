...
class LocalEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.al_encoder_av = ALEncoder(node_dim=node_dim,
                                    ...
                                    num_heads=num_heads,
                                    dropout=dropout)
        self.ag_encoder_av = ALEncoder(node_dim=node_dim,
                                    edge_dim=edge_dim,
                                    embed_dim=embed_dim,
                                    num_heads=num_heads,
                                    dropout=dropout)
        self.al_encoder_agent = ALEncoder(node_dim=node_dim,
                                    ...
                                    num_heads=num_heads,
                                    dropout=dropout)
        self.av_embed = SingleInputEmbedding(in_channel=64, out_channel=embed_dim)
    ...
    def forward(self, data: TemporalData) -> torch.Tensor:
        ...
        edge_index, edge_attr = self.drop_edge_al_route(data['lane_actor_index'], data['lane_actor_vectors'], data['av_index'], data['in_centerline'])
        out_av_centerline = self.ag_encoder_av(x=(data['lane_vectors'], out), edge_index=edge_index, edge_attr=edge_attr,
        ...
        final_av = self.av_embed(out_av_centerline[av_mask] + out_av[av_mask])
        total_out[av_mask] = final_av
