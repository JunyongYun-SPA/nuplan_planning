...
class LocalEncoderAddFeature(nn.Module):
    ...
    def __init__(self,
                 ...
                 local_radius: float = 50,
                 parallel: bool = False) -> None:
        super(LocalEncoderAddFeature, self).__init__()
        self.historical_steps = historical_steps
        self.parallel = parallel
...
class AAEncoderBEV(MessagePassing):
    ...
    def __init__(self,
                 ...
                 parallel: bool = False,
                 **kwargs) -> None:
        super(AAEncoderBEV, self).__init__(aggr='add', node_dim=0, **kwargs)
        self.historical_steps = historical_steps
    ...
    def forward(self,
        ...
        x = x + self._mha_block(x, edge_index, size)
...
class TemporalEncoderCenterline(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_layers: int = 4,
                 dropout: float = 0.1) -> None:
        super(TemporalEncoderCenterline, self).__init__()
        encoder_layer = TemporalEncoderLayer(embed_dim=embed_dim, num_heads=num_heads, dropout=dropout)
        self.transformer_encoder = nn.TransformerEncoder(encoder_layer=encoder_layer, num_layers=num_layers,
                                                         norm=nn.LayerNorm(embed_dim))
        # self.padding_token = nn.Parameter(torch.Tensor(historical_steps, 1, embed_dim))
