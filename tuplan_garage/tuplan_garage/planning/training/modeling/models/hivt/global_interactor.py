...
class GlobalInteractor(nn.Module):
    ...
    def forward(self,
        ...
        rel_pos = data['positions'][edge_index[0], self.historical_steps - 1, :2] - data['positions'][
            edge_index[1], self.historical_steps - 1, :2]
        if data['rotate_mat'] is None:
            rel_embed = self.rel_embed(rel_pos)
        else:
            rel_pos = torch.bmm(rel_pos[:, :2].unsqueeze(-2), data['rotate_mat'][edge_index[1]]).squeeze(-2)
