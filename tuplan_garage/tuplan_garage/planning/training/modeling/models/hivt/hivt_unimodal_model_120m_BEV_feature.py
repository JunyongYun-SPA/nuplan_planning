...
class HiVTModel(TorchModuleWrapper): #JY
    ...
    def forward(self, features: FeaturesType) -> TargetsType:
        ...
        av_point_query_embed = av_point_query_embed[:, input.av_index].unsqueeze(1) #.permute(1, 0, 2) # (B, N, k, D): (32, 16, 64)
        ...
        bev_embed = self.map_embed(bev_embed.reshape(batch_size, -1, 1)) #(32, 40000, 64)
        ...
        agent_embed = bev_agent.reshape(batch_size, 11, -1).permute(1, 0, 2) #(11, 32, 40000)
        ...
        attn_mask = input.bev_map.view(batch_size, 2, 100, 100)[:, 1].reshape(batch_size, -1)
