...
class HiVTModel(TorchModuleWrapper): #JY
    ...
    def forward(self, features: FeaturesType) -> TargetsType:
        ...
        # attn_mask = torch.zeros((batch_size, 40000)).to(av_point_query_embed) #input.bev_map[1]
        
        import matplotlib.pyplot as plt
        fig1, axes1 = plt.subplots(1, 3, figsize=(30, 10))  # 3행 4열의 subplot 생성

        axes1[0].imshow(bev_agent[0, :, :100, :100].cpu().sum(dim=0), cmap='binary')  # 각 층의 occupancy map을 흑백 이미지로 표시
        axes1[0].set_title(f"History")  # subplot 제목 설정
        
        axes1[1].imshow(bev_map[0, :100, :100].cpu(), cmap='binary')  # 각 층의 occupancy map을 흑백 이미지로 표시
        axes1[1].set_title(f"Lane")  # subplot 제목 설정
        
        axes1[2].imshow(attn_mask.view(-1, 100, 100)[0, :100, :100].cpu(), cmap='binary')  # 각 층의 occupancy map을 흑백 이미지로 표시
