...
class LocalEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.drop_edge_aa = DistanceDropEdge(local_radius)
        
        self.drop_edge_al_other_agent = DistanceDropEdgeOtherAgents(local_radius)
        self.drop_edge_al_route = GetInRouteLaneEdgeRoute(local_radius)
        
        self.aa_encoder = AAEncoderExtraFeatures(historical_steps=historical_steps,
                                    node_dim=node_dim,
                                    edge_dim=edge_dim,
                                    embed_dim=embed_dim,
                                    num_heads=num_heads,
        ...
        self.al_encoder_av = ALEncoder(node_dim=node_dim,
                                    edge_dim=edge_dim,
                                    embed_dim=embed_dim,
                                    num_heads=num_heads,
                                    dropout=dropout)
        self.al_encoder_agent = ALEncoder(node_dim=node_dim,
                                    edge_dim=edge_dim,
                                    embed_dim=embed_dim,
                                    num_heads=num_heads,
                                    dropout=dropout)
        self.ag_encoder_av = AGEncoder(node_dim=node_dim,
                                    edge_dim=edge_dim,
                                    embed_dim=embed_dim,
                                    num_heads=num_heads,
                                    dropout=dropout)
    ...
    def forward(self, data: TemporalData) -> torch.Tensor:
        if type(data.av_index) == type(0):
            av_mask = (torch.arange(data.num_nodes).unsqueeze(1) == torch.tensor([data.av_index])).any(dim=1)
        else:
        ...
        for t in range(self.historical_steps):
            if len(data.edge_index) == 0:
                tmp_device = data.edge_index.device
                data.edge_index = torch.tensor([[0],[0]])
                data.edge_index.to(tmp_device)
            data[f'edge_index_{t}'], _ = subgraph(subset=~data['padding_mask'][:, t], edge_index=data.edge_index)
            data[f'edge_attr_{t}'] = \
                data['positions'][data[f'edge_index_{t}'][0], t, :2] - data['positions'][data[f'edge_index_{t}'][1], t, :2]
        if self.parallel:
            snapshots = [None] * self.historical_steps
            for t in range(self.historical_steps):
                edge_index, edge_attr = self.drop_edge_aa(data[f'edge_index_{t}'], data[f'edge_attr_{t}'])
                snapshots[t] = Data(x=data.x[:, t], edge_index=edge_index, edge_attr=edge_attr,
                                    num_nodes=data.num_nodes)
            batch = Batch.from_data_list(snapshots)
            out = self.aa_encoder(x=batch.x[:, :2], t=None, edge_index=batch.edge_index, edge_attr=batch.edge_attr,
            ...
            for t in range(self.historical_steps):
                ...
                out[t] = self.aa_encoder(x=data.x[:, t, :], t=t, edge_index=edge_index, edge_attr=edge_attr,
                                         bos_mask=data['bos_mask'][:, t], rotate_mat=data['rotate_mat'], rotate_angles=data['rotate_angles'])
            out = torch.stack(out)  # [T, N, D]
        out = self.temporal_encoder(x=out, padding_mask=data['padding_mask'][:, : self.historical_steps])
        
        # inter_out = out.new_zeros(out.shape[0], out.shape[1])
        total_out = out.new_zeros(out.shape[0], out.shape[1])
        
        try:
            edge_index, edge_attr = torch.stack((torch.arange(32, device=data.x.device), torch.arange(32, device=data.x.device))), data['y'][data.av_index, -1, :2]
            out_av = self.ag_encoder_av(x=(data['y'][data.av_index, -1, :2], out[data.av_index]), edge_index=edge_index, edge_attr=edge_attr,
                                        rotate_mat=data['rotate_mat'][data.av_index])
        except:
            edge_index, edge_attr = torch.stack((torch.arange(1, device=data.x.device), torch.arange(1, device=data.x.device))), data['y'][data.av_index, -1, :2].unsqueeze(0)
            out_av = self.ag_encoder_av(x=(data['y'][data.av_index, -1, :2].unsqueeze(0), out[data.av_index].unsqueeze(0)), edge_index=edge_index, edge_attr=edge_attr,
                                        rotate_mat=data['rotate_mat'][data.av_index].unsqueeze(0))
        out[av_mask] = out_av
        
        edge_index, edge_attr = self.drop_edge_al_route(data['lane_actor_index'], data['lane_actor_vectors'], data['av_index'], data['in_route_lanes'])
        out_av = self.al_encoder_av(x=(data['lane_vectors'], out), edge_index=edge_index, edge_attr=edge_attr,
        ...
        edge_index, edge_attr = self.drop_edge_al_other_agent(data['lane_actor_index'], data['lane_actor_vectors'], data['av_index'])
        out_agent = self.al_encoder_agent(x=(data['lane_vectors'], out), edge_index=edge_index, edge_attr=edge_attr,
                              is_intersections=data['is_intersections'], turn_directions=data['turn_directions'],
                              traffic_controls=data['traffic_controls'], rotate_mat=data['rotate_mat'])
        
        total_out[av_mask] = out_av[av_mask]
        total_out[other_agent_mask] = out_agent[other_agent_mask]

        return total_out

class AGEncoderExtraFeatures(MessagePassing):
    ...
    def __init__(self,
                 ...
                 parallel: bool = False,
                 **kwargs) -> None:
        super(AGEncoderExtraFeatures, self).__init__(aggr='add', node_dim=0, **kwargs)
        self.historical_steps = historical_steps
        ...
        self.nbr_embed = MultipleInputEmbedding(in_channels=[6, edge_dim], out_channel=embed_dim)
    ...
    def forward(self,
        ...
        if self.parallel:
            ...
            if rotate_mat is None:
                ...
                x_ = torch.bmm(x.unsqueeze(-2)[...,:2], rotate_mat).squeeze()
                angle_embedding = torch.zeros_like(x_)
                if t == 0:
                    relative_angle = x[:, 2]
                else:
                    relative_angle = x[:, 2] - rotate_angles
                relative_angle_ = torch.atan2(torch.sin(relative_angle), torch.cos(relative_angle))
                try:
                    center_embed = self.center_embed(torch.cat((x_, relative_angle_.unsqueeze(-1), x[..., 3:]), dim=-1))
                except:
                    center_embed = self.center_embed(torch.cat((x_.unsqueeze(0), relative_angle_.unsqueeze(-1), x[..., 3:]), dim=-1))
            center_embed = torch.where(bos_mask.unsqueeze(-1), self.bos_token[t], center_embed)
        center_embed = center_embed + self._mha_block(self.norm1(center_embed), x, edge_index, edge_attr, rotate_mat, rotate_angles, t, size)
    ...
    def message(self,
        ...
        if rotate_mat is None:
            ...
            #                             torch.bmm(edge_attr.unsqueeze(-2), center_rotate_mat).squeeze(-2)])
                ...
                center_rotate_mat = rotate_mat[edge_index[1]]
            nbr_x_ = torch.bmm(x_j.unsqueeze(-2)[...,:2], center_rotate_mat).squeeze()
            if t == 0:
                rel_angle = x_j[..., 2]
            else:
                rel_angle = x_j[..., 2] - rotate_angles[edge_index[1]]
            rel_angle_ = torch.atan2(torch.sin(rel_angle), torch.cos(rel_angle))
            try:
                x_j_ = torch.cat((nbr_x_, rel_angle_.unsqueeze(-1), x_j[..., 3:]), dim=-1)
            except:
                x_j_ = torch.cat((nbr_x_.unsqueeze(0), rel_angle_.unsqueeze(-1), x_j[..., 3:]), dim=-1)
            nbr_embed = self.nbr_embed([x_j_,
                                        torch.bmm(edge_attr.unsqueeze(-2), center_rotate_mat).squeeze(-2)])
    ...
    def _mha_block(self,
        ...
        center_embed = self.out_proj(self.propagate(edge_index=edge_index, x=x, center_embed=center_embed,
                                                    edge_attr=edge_attr, rotate_mat=rotate_mat, rotate_angles=rotate_angles, t=t, size=size))
        return self.proj_drop(center_embed)

    def _ff_block(self, x: torch.Tensor) -> torch.Tensor:
        return self.mlp(x)

class AAEncoderExtraFeatures(MessagePassing):
    ...
    def __init__(self,
                 ...
                 parallel: bool = False,
                 **kwargs) -> None:
        super(AAEncoderExtraFeatures, self).__init__(aggr='add', node_dim=0, **kwargs)
        self.historical_steps = historical_steps
        self.embed_dim = embed_dim
        self.num_heads = num_heads
        self.parallel = parallel
...
class AGEncoder(MessagePassing):
    ...
    def __init__(self,
                 ...
                 dropout: float = 0.1,
                 **kwargs) -> None:
        super(AGEncoder, self).__init__(aggr='add', node_dim=0, **kwargs)
        self.embed_dim = embed_dim
        self.num_heads = num_heads
    ...
    def message(self,
        ...
        if rotate_mat is None:
            ...
            rotate_mat = rotate_mat[edge_index[1]] #(32, 2, 2)
            x_j = self.lane_embed([torch.bmm(x_j.unsqueeze(-2).to(torch.float32), rotate_mat.to(torch.float32)).squeeze(-2),
                                   torch.bmm(edge_attr.unsqueeze(-2).to(torch.float32), rotate_mat.to(torch.float32)).squeeze(-2)])
...
class ALEncoderGoal(MessagePassing):
    ...
    def __init__(self,
                 ...
                 dropout: float = 0.1,
                 **kwargs) -> None:
        super(ALEncoderGoal, self).__init__(aggr='add', node_dim=0, **kwargs)
        self.embed_dim = embed_dim
        self.num_heads = num_heads

        self.lane_embed = MultipleInputEmbedding(in_channels=[node_dim, edge_dim, 1], out_channel=embed_dim)
        self.lin_q = nn.Linear(embed_dim, embed_dim)
        self.lin_k = nn.Linear(embed_dim, embed_dim)
        self.lin_v = nn.Linear(embed_dim, embed_dim)
        self.lin_self = nn.Linear(embed_dim, embed_dim)
        ...
        self.turn_direction_embed = nn.Parameter(torch.Tensor(3, embed_dim))
        self.traffic_control_embed = nn.Parameter(torch.Tensor(2, embed_dim))
        self.goal_embed = nn.Parameter(torch.Tensor(2, embed_dim))
        nn.init.normal_(self.is_intersection_embed, mean=0., std=.02)
        nn.init.normal_(self.turn_direction_embed, mean=0., std=.02)
        nn.init.normal_(self.traffic_control_embed, mean=0., std=.02)
        self.apply(init_weights)
    ...
    def forward(self,
        ...
        goals = goals.float()
        x_actor = x_actor + self._mha_block(self.norm1(x_actor), x_lane, edge_index, edge_attr, is_intersections,
                                            turn_directions, traffic_controls, goals, rotate_mat, size)
        x_actor = x_actor + self._ff_block(self.norm2(x_actor))
        return x_actor
    ...
    def _mha_block(self,
        ...
        x_actor = self.out_proj(self.propagate(edge_index=edge_index, x=(x_lane, x_actor), edge_attr=edge_attr,
                                               is_intersections=is_intersections, turn_directions=turn_directions,
                                               traffic_controls=traffic_controls, goals=goals, rotate_mat=rotate_mat, size=size))
        return self.proj_drop(x_actor)
