...
def waypoints_to_trajectory(
    ...
    stationary_points = waypoints[:, :, 0] < 0.2 #(1, 16) # 0.2미만으로 움직였을 때는 waypoints를 0으로 덮어씌움
    waypoints[stationary_points] = 0.0
    waypoints = torch.cat([initial_position, waypoints], dim=1) # 예측 위치에 대해 현재 위치 정보를 추가
    # need to move waypoints to cpu for interpolation. Original tensor is kept to preserve gradients.
    current_velocity_cpu = current_velocity #.detach().cpu() # 현재 차량의 속도
    ...
    ) # point와 point 사이를 100개로 slice, ([1602])
    output_slice = slice(
        0, (num_waypoints - 1) * supersampling_ratio + 1, supersampling_ratio
    ) # 추후 100개의 단위마다 다시 trajectory를 뽑아서 사용하기 위함
    ...
    for sample_idx in range(batch_size):
        x_interp_func = make_interp_spline(
            ...
            k=3,
            bc_type=([(1, current_velocity_cpu[sample_idx])], [(2, 0.0)]), # bc_type은 spline의 미분의 차수와 그 값을 직접 지정할 수 있음
        )
