...
class HiVTFeatureBuilder(AbstractFeatureBuilder): #JY
    ...
    def _compute_feature(
        ...
        center_heading = torch.tensor(scenario.initial_ego_state.center.heading).unsqueeze(0)
        ...
        #         agent_pos = x.squeeze(0)[node_idx, valid_hist_t]
        #         pos_xs = agent_pos[:, 0]
        #         pos_ys = agent_pos[:, 1]
        ...
        # plt.scatter(lane_x, lane_y, s = 15, c="green", alpha=1)
        ...
        # plt.scatter(lane_x, lane_y, s = 20, c="green", alpha=1)
        ...
        # y = xx_[:, 11:]
        # xxx_ = xx_[:, :11]
        # xxxx_ = torch.cumsum(xxx_, axis=1)
        # # '37685ae6108752e0'
        # # y 회전 검증
        # # y의 경우 0,0에서 offset으로 예측되기 때문에 굳이 추가적인 작업을 해줄 필요는 없다
        # x 도 동일하다
        # rotate_mat = torch.empty(num_nodes, 2, 2)
        ...
        #     rect.set_transform(t)
        #     ax.add_patch(rect)
        # plt.savefig(f"/home/workspace/pictures/{scenario.token}_y_{node_idx}.PNG")

        # # x 검증
        # xxxx_[:,:,:2] = torch.bmm(xxxx_[:, :, :2], rotate_mat)
        # node_idx = 0
        # plt.clf()
        # plt.figure(figsize=(26,26))
        # plt.xlim(-20, 60)
        # plt.ylim(-60, 60)
        # pos_xs = xxxx_[node_idx, :, 0]
        # pos_ys = xxxx_[node_idx, :, 1]
        # pos_yaws = xxxx_[node_idx, :, 2]
        # for index_ in range(pos_xs.shape[0]):
        #     pos_x = pos_xs[index_]
        #     pos_y = pos_ys[index_]
        #     pos_yaw = pos_yaws[index_]
        ...
        #     rect.set_transform(t)
        #     ax.add_patch(rect)
        # plt.savefig(f"/home/workspace/pictures/{scenario.token}_x_{node_idx}.PNG")
