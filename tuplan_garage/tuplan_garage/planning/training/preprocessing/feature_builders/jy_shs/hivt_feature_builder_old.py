...
class HiVTFeatureBuilder(AbstractFeatureBuilder): #JY
    ...
    def get_features_from_scenario(self, scenario: AbstractScenario) -> HiVTFeature: #JY
    ...
    def _compute_feature(
        ...
        (lane_vectors, is_intersections, turn_directions, traffic_controls, lane_actor_index,
         lane_actor_vectors) = self.get_lane_features(initialization.map_api, node_positions, origin, rot_mat, radius)
        ...
        return HiVTFeature( #JY
            ...
            neighbor_position=neighbor_position, #JY
            planner_centerline=planner_centerline,
            planner_trajectory=planner_trajectory,
        )

    def get_lane_features(self, map_api, node_positions, origin, rot_mat, radius):
        lane_positions, lane_vectors, is_intersections, turn_directions, traffic_controls = [], [], [], [], []
        lane_ids = set()
        for node_position in node_positions:
            print()
            position: Point2D = node_position.center.point
            LANE_obj = map_api.get_proximal_map_objects(node_position[0], node_position[1], city, radius)
            lane_ids.update(map_api.get_proximal_map_objects(node_position[0], node_position[1], city, radius))
