...
class HiVTFeatureBuilder(AbstractFeatureBuilder): #JY
    ...
    def _compute_feature(
        ...
        static_obstacle_tracks_barrier = self.filter_tracked_objects_by_type(ALL_observation, TrackedObjectType.BARRIER)
        static_obstacle_tracks_czone_sign = self.filter_tracked_objects_by_type(ALL_observation, TrackedObjectType.CZONE_SIGN)
        static_obstacle_tracks_generic_object = self.filter_tracked_objects_by_type(ALL_observation, TrackedObjectType.GENERIC_OBJECT)
        static_obstacle_tracks_traffic_cone = self.filter_tracked_objects_by_type(ALL_observation, TrackedObjectType.TRAFFIC_CONE)
        
        for s_c in vehicle_tracks:
            for agent in s_c.tracked_objects.tracked_objects:
                actor_ids.append(agent.track_token)

        for s_c in static_obstacle_tracks_barrier:
            for agent in s_c.tracked_objects.tracked_objects:
                static_actor_ids.append(agent.track_token)
        for s_c in static_obstacle_tracks_czone_sign:
            for agent in s_c.tracked_objects.tracked_objects:
                static_actor_ids.append(agent.track_token)
        for s_c in static_obstacle_tracks_generic_object:
            for agent in s_c.tracked_objects.tracked_objects:
                static_actor_ids.append(agent.track_token)
        for s_c in static_obstacle_tracks_traffic_cone:
            for agent in s_c.tracked_objects.tracked_objects:
                static_actor_ids.append(agent.track_token)

        actor_ids = list(set(actor_ids))
        static_actor_ids = list(set(static_actor_ids))
        num_nodes = len(actor_ids)
        num_nodes_static = len(static_actor_ids)
        ...
        x_static = torch.zeros(num_nodes_static, 27, 2, dtype=torch.float) # 9, 10, 16, arr[:10]-> hist, arr[10]-> current, arr[10:16] -> fut
        padding_mask_static = torch.ones(num_nodes_static, 27, dtype=torch.bool)
        ...
        # edge_index_static = torch.LongTensor(list(permutations(range(num_nodes_static), 2))).t().contiguous()
        edge_index_static = torch.LongTensor(torch.stack((torch.arange(num_nodes_static) ,torch.tensor(av_index).repeat(num_nodes_static))))
        ...
        for step, s_c in enumerate(static_obstacle_tracks_barrier):
            for agent in s_c.tracked_objects.tracked_objects:
                node_idx = static_actor_ids.index(agent.track_token)
                padding_mask_static[node_idx, step] = False
                x_static[node_idx, step] = torch.Tensor([agent.center.x, agent.center.y])
        for step, s_c in enumerate(static_obstacle_tracks_czone_sign):
            for agent in s_c.tracked_objects.tracked_objects:
                node_idx = static_actor_ids.index(agent.track_token)
                padding_mask_static[node_idx, step] = False
                x_static[node_idx, step] = torch.Tensor([agent.center.x, agent.center.y])
        for step, s_c in enumerate(static_obstacle_tracks_generic_object):
            for agent in s_c.tracked_objects.tracked_objects:
                node_idx = static_actor_ids.index(agent.track_token)
                padding_mask_static[node_idx, step] = False
                x_static[node_idx, step] = torch.Tensor([agent.center.x, agent.center.y])
        for step, s_c in enumerate(static_obstacle_tracks_traffic_cone):
            for agent in s_c.tracked_objects.tracked_objects:
                node_idx = static_actor_ids.index(agent.track_token)
                padding_mask_static[node_idx, step] = False
                x_static[node_idx, step] = torch.Tensor([agent.center.x, agent.center.y])
        ...
        for i in range(len(static_actor_ids)):
            if padding_mask_static[i, 10]:  # make no predictions for actors that are unseen at the current time step
                padding_mask_static[i, 11:] = True
        ...
        x_static = x_static.clone().view(1, num_nodes_static, num_timestamps, 2).repeat(num_center_objects, 1, 1, 1)
        x[:, :, :, 0:center_xyz.shape[1]] -= center_xyz[:, None, None, :]
        x_static[:, :, :, 0:2] -= center_xyz[:, None, None, :2]
        ...
        x = x.squeeze(0)
        x_static[:, :, :, 0:2] = rotate_points_along_z(points=x_static[:, :, :, 0:2].view(num_center_objects, -1, 2),
                                                        angle=-center_heading).view(num_center_objects, num_nodes_static, num_timestamps, 2)
        x_static = x_static.squeeze(0)
        ...
        (lane_vectors, is_intersections, turn_directions, traffic_controls, traffic_controls_red, lane_actor_index,
         lane_actor_vectors, lane_positions, goals, in_route_lanes, in_centerline, traffic_red_polygon, route_polygon) = \
             self.get_lane_features(current_input, initialization.map_api, node_inds, node_positions, origin, rotate_mat, radius, end_lanes, start_lanes, \
                 av_index=actor_ids.index('ego'), planner_centerline=planner_centerline)
        ...
        occupancy_size = 200
        occupancy_radius = 100
        occupancy_range = int(occupancy_size * 0.5)
        occupancy_map_env = torch.zeros(5, occupancy_size+1, occupancy_size+1, dtype=torch.float32)
        occupancy_map_history = torch.zeros(11, occupancy_size+1, occupancy_size+1, dtype=torch.float32)
        
        drop_edge_av = DistanceDropEdge(occupancy_radius)
        # drop_edge_route = GetInRouteLaneEdgeRoute(occupancy_radius)
        lane_edge_index, lane_attr_origin = drop_edge_av(lane_actor_index, lane_actor_vectors)
        # route_edge_index, route_attr_origin = drop_edge_route(lane_actor_index, lane_actor_vectors, av_index, in_route_lanes)
        ...
        occ_mask_lane = (abs(lane_attr[:, 0]) < occupancy_radius) * (abs(lane_attr[:, 1]) < occupancy_radius)
        lane = torch.where(((occ_mask_lane).unsqueeze(-1)),
                                lane_attr,
                                occupancy_radius*torch.ones(lane_attr.shape[0], 2).to(lane_attr.device))
        ...
        # route_mask = (route_edge_index[1]==av_index) 
        # route_mask_index = torch.where(route_mask ==True)[0] #torch.Size([1419])
        # route_attr = route_attr_origin[route_mask_index] + positions[av_index, 10, :2]

        # occ_mask_route = (abs(route_attr[:, 0]) < occupancy_radius) * (abs(route_attr[:, 1]) < occupancy_radius)
        # route = torch.where(((occ_mask_route).unsqueeze(-1)),
        #                         route_attr,
        #                         occupancy_radius*torch.ones(route_attr.shape[0], 2).to(route_attr.device))
        # occupancy_x_route, occupancy_y_route = \
        #             (route / occupancy_resolution).type(torch.int)[:, 0], \
        #                 (route / occupancy_resolution).type(torch.int)[:, 1]
        # occupancy_y_route[~(occ_mask_route)] = \
        #     -1 * occupancy_y_route[~(occ_mask_route)]
        # occupancy_map_env[5, \
        #         (occupancy_range - occupancy_y_route).type(torch.long), \
        #             (occupancy_x_route + occupancy_range).type(torch.long)] = 1
        
        static_history = x_static[:, 10, :2] #(456, 11, 2)
        occ_mask_static = (abs(static_history[:, 0]) < occupancy_radius) * (abs(static_history[:, 1]) < occupancy_radius)
        static = torch.where((((~padding_mask_static[:, 10]) * occ_mask_static).unsqueeze(-1)),
                                static_history,
                                occupancy_radius*torch.ones(static_history.shape[0], 2).to(static_history.device))
        ...
        occupancy_y_static[~((~padding_mask_static[:, 10]) * occ_mask_static)] = \
            -1 * occupancy_y_static[~((~padding_mask_static[:, 10]) * occ_mask_static)]
        occupancy_map_env[2, \
                (occupancy_range - occupancy_y_static).type(torch.long), \
                    (occupancy_x_static + occupancy_range).type(torch.long)] = 1
        
        # traffic_red_mask = torch.where(traffic_controls_red == 1)[0]
        # traffic_red_mask_total = (lane_edge_index[1].unsqueeze(1)==av_index).any(dim=1)  * (lane_edge_index[0].unsqueeze(1)==traffic_red_mask).any(dim=1)
        # lane_red_mask_index = torch.where(traffic_red_mask_total==True)[0]
        # lanes_red_indx = lane_edge_index[1][lane_red_mask_index]
        # lane_attr_red = lane_attr_origin[lane_red_mask_index] + positions[lanes_red_indx, 10, :2]
        ...
        try:
            ...
            occ_mask = (abs(agent_history[:, :, 0]) < occupancy_radius) * (abs(agent_history[:, :, 1]) < occupancy_radius)
            
            agent_history = torch.where((((~padding_mask[others_indx, :11]) * occ_mask).unsqueeze(-1)),
                                    agent_history,
                                    occupancy_radius*torch.ones(others_indx.shape[0], 11, 2).to(agent_history.device))
        ...
        for occ_x in range(occupancy_size):
            for occ_y in range(occupancy_size):
                ...
                # drivable_area_polygon = list(scenario.map_api._get_vector_map_layer(SemanticMapLayer.DRIVABLE_AREA).values[:, 0])
                # is_onroad = np.any(np.array(points_in_polygons_v2(global_xy.unsqueeze(0), drivable_area_polygon)))
                is_onroad = scenario.map_api.is_in_layer(point, layer=SemanticMapLayer.DRIVABLE_AREA)
                # is_onred = np.any(np.array(points_in_polygons(global_xy, traffic_red_polygon)))
                is_onred = points_in_polygons_v2(global_xy, traffic_red_polygon)
                is_onroute = points_in_polygons_v2(global_xy, route_polygon)
                if is_onroad:
                    occupancy_map_env[1, occ_y, occ_x] = 1   
                if is_onred:
                    occupancy_map_env[3, occ_y, occ_x] = 1
                if is_onroute:
                    occupancy_map_env[4, occ_y, occ_x] = 1
        # occupancy_map_env[3, :, :] += (~occupancy_map_env[1, :, :].to(torch.bool)).to(torch.int)
        
        # import matplotlib.pyplot as plt
        # fig1, axes1 = plt.subplots(1, 6, figsize=(60, 10))  # 3행 4열의 subplot 생성
        ...
        # axes1[2].set_title(f"RoadBlock")  # subplot 제목 설정
        
        # axes1[3].imshow(occupancy_map_env[2, :occupancy_size, :occupancy_size], cmap='binary')  # 각 층의 occupancy map을 흑백 이미지로 표시
        # axes1[3].set_title(f"Static")  # subplot 제목 설정
        
        # axes1[4].imshow(occupancy_map_env[3, :occupancy_size, :occupancy_size], cmap='binary')  # 각 층의 occupancy map을 흑백 이미지로 표시
        # axes1[4].set_title(f"Traffic_light")  # subplot 제목 설정
        
        # axes1[5].imshow(occupancy_map_env[4, :occupancy_size, :occupancy_size], cmap='binary')  # 각 층의 occupancy map을 흑백 이미지로 표시
        # axes1[5].set_title(f"Route_mask")  # subplot 제목 설정
        
        # # # occupancy_map_env[3, :, :][(~occupancy_map_env[4, :, :].to(torch.bool))] = 1
        # # axes1[6].imshow(occupancy_map_env[3, :occupancy_size, :occupancy_size], cmap='binary')  # 각 층의 occupancy map을 흑백 이미지로 표시
        # # axes1[6].set_title(f"Lane")  # subplot 제목 설정

        # plt.tight_layout()  # subplot 간격 조정
        # plt.savefig(f'/home/workspace/tuplan_garage/tuplan_garage/planning/training/preprocessing/feature_builders/vis/{scenario.token}.png')
        ...
        result = {
            ...
            'static_positions': x_static,
            'edge_index':edge_index,
            'edge_index_static': edge_index_static,
            'y': y, # y.shape(15, 16, 3)
            ...
            'traffic_controls_red': traffic_controls_red,
    ...
    def get_lane_features(self, current_input, map_api, node_inds, node_positions, origin, rot_mat, radius, end_lanes, start_lanes, av_index, planner_centerline):
        lane_positions, lane_vectors, is_intersections, turn_directions, traffic_controls, traffic_controls_red, goals, lane_centerlines, in_route_lanes, in_centerline = [], [], [], [], [], [], [], [], [], []
        lane_ids = set()
        lane_con_ids = set()
        ...
        traffic_lane_con_ids = {data.lane_connector_id: data.status for data in current_input.traffic_light_data}
        ...
        for lane_id in lane_ids:
            ...
            traffic_controls_red.append(traffic_red_lane * torch.ones(count, dtype=torch.uint8))
            # goal
            ...
            if lane_id in end_lanes["route_lane_ids"]:
                if lane_id in start_lanes["Lane_ids"]:
                    ...
                    route_polygon.append(lane_obj.polygon)
                else:
        ...
        for lane_id in lane_con_ids:
            ...
            lane_centerline = torch.matmul(lane_centerline - origin, rot_mat)
            if int(lane_id) in list(traffic_lane_con_ids.keys()):
                traffic_control = True 
                if traffic_lane_con_ids[int(lane_id)] == TrafficLightStatusType.RED:
                    traffic_red_lane = True
                    traffic_red_polygon.append(lane_obj.polygon)
                elif traffic_lane_con_ids[int(lane_id)] == TrafficLightStatusType.GREEN:
                    traffic_red_lane = False
            else:
                traffic_control = False 
                traffic_red_lane = False
        ...
        traffic_controls_red = torch.cat(traffic_controls_red, dim=0)
        ...
        return lane_vectors, is_intersections, turn_directions, traffic_controls, traffic_controls_red, lane_actor_index, lane_actor_vectors, lane_positions, goals, in_route_lanes, in_centerline, traffic_red_polygon, route_polygon
