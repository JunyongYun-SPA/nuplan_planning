...
class HiVTFeatureBuilder(AbstractFeatureBuilder): #JY
    ...
    def _compute_feature(
        ...
        if scenario.token == "024d034d9cd25bf3":
            import matplotlib.pyplot as plt
            import matplotlib.patches as patches
            
            cur_t = 11
            hist_t = list(range(0, 27))
            ...
            local_route_polygon = []
            for id_ in cliped_route_ids: #initialization.route_roadblock_ids:
                try:
                    poly = initialization.map_api._get_roadblock_connector(id_).polygon
                except Exception as e:
                    poly = initialization.map_api._get_roadblock(id_).polygon
                x_ = list(poly.exterior.xy[0])
                y_ = list(poly.exterior.xy[1])
                arr_ = torch.tensor([x_, y_]).T
                local_arr_ = torch.matmul(arr_ - origin, rotate_mat)
            ...
            for index, local_route_ in enumerate(local_route_polygon):
                x_, y_ = local_route_.exterior.xy
                if index == 0:
                    plt.fill(x_, y_, color='gold', alpha=0.2)
                else:
                    plt.fill(x_, y_, color='gold', alpha=0.2)
            
            center_x = planner_centerline[:, 0]   
            center_y = planner_centerline[:, 1]   
            plt.scatter(center_x, center_y, s = 25, c="Cyan", alpha=0.3)
            ...
            plt.savefig(f"/home/workspace/visualization/{scenario.token}.PNG")
