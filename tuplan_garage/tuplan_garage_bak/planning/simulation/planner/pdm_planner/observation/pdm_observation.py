...
class PDMObservation:
    ...
    def update(
        ...
        self._object_manager = self._get_object_manager(ego_state, observation) # observation으로부터 추적/고정 object와 각 object의 좌표를 추적한다

        (
            traffic_light_tokens,
            traffic_light_polygons,
        ) = self._get_traffic_light_geometries(traffic_light_data, route_lane_dict) # traffic light 정보를 가져온다

        (
            static_object_tokens, # 길이 12개짜리 리스트
            static_object_coords, # (12, 5, 2) current state를 box로 표현한것 5개인 이유는 centerline이 포함되어 있음
            dynamic_object_tokens, # 길이 53개짜리 리스트
            dynamic_object_coords, # (53, 5, 2)current state를 box로 표현한것 5개인 이유는 centerline이 포함되어 있음
            dynamic_object_dxy, #(53, 2) current state dxy
        ) = self._object_manager.get_nearest_objects(ego_state.center.point) # ego의 현재 위치에서 가장 가까운 object를 하나씩 가져온다
        # BH 시각화 테스트
        # import matplotlib.pyplot as plt
        # box = static_object_coords[0,...]
        # plt.scatter(box[:, 0], box[:, 1])
        # plt.savefig("/home/workspace/tmp_fig.png")
        ...
        # Occupancy map의 현재 시점에서 충돌하는 물체를 식별하고, 저장한다
