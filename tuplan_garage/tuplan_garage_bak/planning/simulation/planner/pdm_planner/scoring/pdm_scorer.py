...
class PDMScorer:
    ...
    def _calculate_ttc(self):
        ...
        future_time_idcs = np.arange(0, 10, 3) # (0, 3, 6, 9)
        n_future_steps = len(future_time_idcs) # 4

        # create polygons for each ego position and 1s future projection
        coords_exterior = self._ego_coords.copy() # ~(15, 41(all_time step), 5(box), 2(x,y))
        ...
        polygons = creation.polygons(coords_exterior_time_steps) #(15, 41, 4(0, 3, 6, 9), 5(box), 2(x,y)) 

        # check collision for each proposal and projection
        for time_idx in range(self._proposal_sampling.num_poses + 1):
            for step_idx, future_time_idx in enumerate(future_time_idcs): # (0, 3, 6, 9)
                ...
                ) # (2, 11) 하나는 충돌하는 ego의 index를 다른 하나는 충돌하는 observation 객체의 index를 나타내나?
                ...
                for proposal_idx, geometry_idx in zip(intersecting[0], intersecting[1]):
                    ...
                    ): # 충돌하는 object가 신호거나, 자기자신의 track이거나, proposal가 정지해있는 경우
                        continue
                    ...
                    ): # 조건이 좀 까다로운데, 충돌하는 물체의 좌표를 track_state, ego의 좌표를 ego_rear_axle이라 하자, 
                        # 만약 ego rear axle이 track state보다 뒤쪽에 있을 경우 ttc_score는 0으로 기록된다.
                        # 또는 ego가 multilane에 거쳐있거나 드라이브 area에 없음 혹은 intersection에 있음이라는 조건에서
                        # ego rear axle이 충돌한 물체보다 뒤쪽에 있을 경우 ttc score는 0으로 기록된다.
