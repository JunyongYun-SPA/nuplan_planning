...
class PDMGenerator:
    ...
    def generate_proposals(
        ...
        lateral_batch_dict = self._get_lateral_batch_dict() # {0: [0, 1, 2, 3, 4], 1: [5, 6, 7, 8, 9], 2: [10, 11, 12, 13, 14]}

        for lateral_idx, lateral_batch_idcs in lateral_batch_dict.items():
            self._initialize_states(lateral_batch_idcs) # 몇번 레인이느냐에 따라 초기화를 다르게 진행, 주요 역할은 progress를 구하고, 각 lane에 progress를 projection시켜 위치를 구해내는 것
            for time_idx in range(1, self._proposal_sampling.num_poses + 1, 1): # self._proposal_sampling.num_poses = 40
                self._update_leading_agents(lateral_batch_idcs, time_idx) # 앞서가는 차량을 식별하고, self._leading_agent_array를 채운다
    ...
    def _initialize_states(self, lateral_batch_idcs: List[int]) -> None:
        ...
        dummy_proposal_idx = lateral_batch_idcs[0] # 5개의 속도 파라미터 중 첫번째 것을 가져옴

        ego_position = Point(*self._initial_ego_state.rear_axle.point.array) # 현재 iteration에서의 ego차량의 위치를 가져옴

        ego_progress = self._proposal_manager[dummy_proposal_idx].linestring.project(
            ego_position
        ) # 25m 정도 -> 이는 몇번째 proposal manager인지에 관계없이 무조건 현재 차량과 centerline과의 진행도를 가져오는 것
        ego_velocity = self._initial_ego_state.dynamic_car_state.rear_axle_velocity_2d.x # 현재 iteration에서 ego의 속도를 가져옴
        ...
        ] = ego_velocity # longitude lane 하나당, lateral_batch 5개가 존재하며, 이들에 해당하는 self._state_idm_array의 올바른 위치에 ego initial_state의 progress와 velocity를 추가함

        state_array = self._proposal_manager[dummy_proposal_idx].path.interpolate(
            [ego_progress], as_array=True
        )[0] # (3, ), 현재 lane에서 progress를 projection했을 때 위치가 어디냐? 를 가져옴
        self._state_array[lateral_batch_idcs, 0, StateIndex.STATE_SE2] = state_array # self._state_array의 해당되는 위치에 앞서 가져온 값을 채움
    ...
    def _update_leading_agents(
        ...
        if not update_leading_agent:
            ...
            leading_agent_array = np.zeros(len(LeadingAgentIndex), dtype=np.float64) # (3,)
            intersecting_objects: List[str] = self._get_intersecting_objects(
                lateral_batch_idcs, time_idx
            ) # lateral_batch_idcs=[0,1,2,3,4] / time_idx=2, 현재 current_lane을 따라 최고속도로 8s까지 확장한 polygon을 이용하여 현재 occupancy에서 충돌하는 물체들의 리스트를 모두 반환한다
            ...
            for object in intersecting_objects:
                if object not in self._observation.collided_track_ids:
                    ...
                    object_progress_dict[object] = object_progress # 각 object의 위치를 centerline(혹은 parreal line)에 projection시켰을 때의 progress를 구한다
            ...
            for proposal_idx in lateral_batch_idcs:
                ...
                } # 현재 ego보다 앞서 있는 object만 가져온다

                if len(agents_ahead) > 0:  # red light, object or agent ahead # 선두에 있는 객체의 상대적 거리와 속도를 계산하여 선두 차량 배열에 저장
                                           # 빨간 불인 경우에는 속도를 0으로 두고 선두차량으로 두는 것 같음
                ...
                self._leading_agent_array[proposal_idx, time_idx] = leading_agent_array # 선두 차량의 <LEN / PROGRESS / VEL>을 계산하고 self._leading_agent_array에 저장한다
    ...
    def _get_intersecting_objects(
        ...
        """
        dummy_proposal_idx = lateral_batch_idcs[0]
        driving_corridor: Polygon = self._get_driving_corridor(dummy_proposal_idx) # 주어진 centerline을 따라 8s 최고 속도로 달렸을 때 생성되는 polygon 의미
        return self._observation[time_idx].intersects(driving_corridor) # 이 polygon과 충돌하는 객체를 현재 occupancy map에서 찾아서 반환 
