...
class AbstractPDMClosedPlanner(AbstractPDMPlanner):
    ...
    def _update_proposal_manager(self, ego_state: EgoState):
        ...
        if create_new_proposals:
            proposal_paths: List[PDMPath] = self._get_proposal_paths(current_lane) # centerline_lane과 [offset=-1.0, 1.0]인 parralel lane 2개가 존재함, 여기서 말하는 centerline은 point 3599개를 가지는 가공되지 않은 centerline이 됨

            self._proposal_manager = PDMProposalManager(
                lateral_proposals=proposal_paths,
                longitudinal_policies=self._idm_policies,
            ) # proposal_path와 idm policy class객체를 담아서 저장
    ...
    def _get_closed_loop_trajectory(
        ...
        # self._proposal_magager._proposals = [15 X PDMPROPOSAL]
        ...
        if trajectory is None:
            trajectory = self._generator.generate_trajectory(np.argmax(proposal_scores)) # 하

        # Scoring에 따라 single 하나만 return
        return trajectory
