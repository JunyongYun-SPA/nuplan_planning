...
class PDMOpenModel(TorchModuleWrapper):
    ...
    def __init__(
        ...
        self.state_encoding = nn.Sequential(nn.Linear((history_sampling.num_poses + 1) * 3 * len(SE2Index), self.hidden_dim),
                                            nn.ReLU())
    ...
    def forward(self, features: FeaturesType) -> TargetsType:
        ...
        state_features = torch.cat([ego_position, ego_velocity, ego_acceleration], dim=-1) # (64<batch>, 99(11*3))의 모양을 지닌다.BH
        ...
        output_trajectory = output_trajectory.reshape(batch_size, -1, len(SE2Index)) # len(SE2Index) = 3, BH
