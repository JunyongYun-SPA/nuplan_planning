...
def cluster_and_rank(k: int, data: np.ndarray, random_state: int = None):
    ...
    def rank_clusters(cluster_counts, cluster_centers):
        ...
        즉 ward를 기준으로 클러스터 랭킹을 매긴다는 말이다.
        1. 초기설정
        함수는 클러스터 수, 클러스터 ID, 순위를 초기화합니다. 순위는 모든 클러스터에 대해 1로 설정됩니다.
        2. Ward 거리 계산
        각 클러스터 센터 간의 거리(centroid_dists)를 계산합니다. 이는 클러스터 센터 간의 유클리디안 거리를 나타냅니다.
        각 클러스터의 데이터 포인트 수(n1, n2)를 기반으로 Ward 거리를 계산합니다. Ward 거리는 두 클러스터를 합쳤을 때 증가하는 분산의 양으로, 두 클러스터의 데이터 포인트 수와 센터 간 거리에 의존합니다.
        dists 배열은 각 클러스터 쌍에 대한 Ward 거리를 포함합니다. 대각선은 무한대로 설정되어 자기 자신과의 결합을 방지합니다.
        3. 클러스터 병합 및 순위 할당
        가장 작은 Ward 거리를 가진 클러스터 쌍을 찾습니다(c1, c2).
        두 클러스터 중 데이터 포인트 수가 더 적은 클러스터를 선택합니다(c). 선택된 클러스터는 현재 순위(i)를 받습니다.
        선택된 클러스터(c)와 다른 클러스터(c_)를 병합합니다. 병합된 새 클러스터의 센터와 카운트를 업데이트합니다.
        병합에 사용된 클러스터(c)를 제거하고, 남은 클러스터에 대해 위의 과정을 반복합니다.
        """
...
def cluster_traj(
    ...
    data = traj[:, :, 0::3, :] # 다운 샘플링 traj 즉 16개의 horizon을 모두 사용하는 대신, 3개씩 건너뛰어 6개의 horizon만 사용 (1, 1000, 16, 2) -> (1, 1000, 6, 2)
    data = data.reshape(batch_size, num_samples, -1).detach().cpu().numpy() # (1, 1000, 12)
    ...
    cluster_lbls = np.array([cluster_op["lbls"] for cluster_op in cluster_ops]) # 각 trajectory가 할당된 클러스터 레이블을 의미(1, 1000)
    cluster_counts = np.array([cluster_op["counts"] for cluster_op in cluster_ops]) # 각 클러스터 레이블이 가지는 trajectory 객체의 개수(1, 10)
    cluster_ranks = np.array([cluster_op["ranks"] for cluster_op in cluster_ops]) # ward법에 따라 랭크가 매겨진 클러스터(1, 10)
    ...
    traj_summed = torch.zeros(batch_size, k, traj_len, 2, device=device).scatter_add(
        1, lbls, traj
    ) # (1, 10, 16, 2), 각 클러스터에 속하는 트라젝토리 포인트들의 합계를 계산한다.
    cnt_tensor = (
        torch.as_tensor(cluster_counts, device=device)
        .unsqueeze(-1)
        .unsqueeze(-1)
        .repeat(1, 1, traj_len, 2)
    ) # (1, 10, 16, 2) 각 클러스터에 속하는 trajectory의 개수
    traj_clustered = traj_summed / cnt_tensor # 중심점을 구한다.
    scores = 1 / torch.as_tensor(cluster_ranks, device=device) # 랭크의 역순으로 score를 구함 최종적으로 score는 클러스터의 밀집도와 구분성이 높을수록 높은 점수를 받게 된다.
