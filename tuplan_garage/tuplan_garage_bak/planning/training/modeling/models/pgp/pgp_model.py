...
class PGPModel(TorchModuleWrapper):
    ...
    def forward(self, features: Dict[str, PGPFeatures]) -> TargetsType:
        ...
        if self.average_output_trajectories: # False
            most_likely_trajectory = outputs["traj_none_clustered"].mean(dim=1)
        else:
            if self.filter_trajectories_by_endpoint: # False
        ...
        if self.smooth_output_trajectory: # False
            most_likely_trajectory = smooth_centerline_trajectory(
                most_likely_trajectory
            )

        if self.return_traversal_coordinates: # true
            ...
            ) # (1, 1000, 16, 2) traversal의 coordinate를 계산한다.
        else:
        ...
        if self.return_graph_map: # False
