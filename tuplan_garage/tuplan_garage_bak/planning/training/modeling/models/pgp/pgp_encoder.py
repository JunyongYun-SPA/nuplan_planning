...
class PGPEncoder(nn.Module):
    ...
    def forward(self, inputs: Dict) -> Dict:
        ...
        # Encode target agent
        target_agent_feats = inputs.ego_agent_features.ego_feats.squeeze(1).float() # (1, 6, 5)
        target_agent_embedding = self.leaky_relu(self.target_agent_emb(target_agent_feats)) #(1, 6, 16)
        _, target_agent_enc = self.target_agent_enc(target_agent_embedding) # GRU를 통해 time축을 따라 encoding -> (1, 1, 32)
        target_agent_enc = target_agent_enc.squeeze(0)

        # Encode lane nodes
        lane_node_feats = inputs.graph_map.lane_node_feats # (1, 70, 20, 7)
        if self.use_route_feature: # False로 설정되어 있음
            on_route_flag = inputs.graph_map.nodes_on_route_flag
            lane_node_feats = torch.cat([lane_node_feats, on_route_flag], dim=-1)
        if self.use_red_light_feature: # True로 설정되어 있음
            red_light_flag = inputs.graph_map.red_light_flag
            lane_node_feats = torch.cat([lane_node_feats, red_light_flag], dim=-1) # (1, 70, 20, 8)
        lane_node_masks = inputs.graph_map.lane_node_masks
        lane_node_embedding = self.leaky_relu(self.node_emb(lane_node_feats)) # (1, 70, 20, 16)
        ...
        nbr_vehicle_feats = inputs.ego_agent_features.vehicle_agent_feats # (1, 34, 6, 5)
        ...
        nbr_vehicle_embedding = self.leaky_relu(self.nbr_emb(nbr_vehicle_feats)) # (1, 34, 6, 16)
