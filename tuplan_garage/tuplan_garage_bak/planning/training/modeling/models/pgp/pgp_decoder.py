...
class LVM(nn.Module):
    ...
    def forward(self, inputs: Union[Dict, torch.Tensor]) -> Dict:
        ...
        if type(inputs) is torch.Tensor:
            ...
            agg_encoding = inputs["agg_encoding"] # (1, 1000, 160)

        if self.agg_type == "combined": # self.agg_type == 'sample_specific'
        ...
        batch_size = agg_encoding.shape[0] # 1
        z = torch.randn(
            batch_size, self.num_samples, self.lv_dim, device=agg_encoding.device
        ) # (1, 1000, 5)
        agg_encoding = torch.cat((agg_encoding, z), dim=2) # (1, 1000, 165)
        h = self.leaky_relu(self.hidden(agg_encoding)) # MLP 연산, 1, 1000, 128

        # Output trajectories
        traj = self.op_traj(h) # (1, 1000, 128) -> (1, 1000, 32)
        traj = traj.reshape(batch_size, self.num_samples, self.op_len, 2) # (1, 1000, 32) -> (1, 1000, 16 2)
        ...
        # pi와 sampled_traversals 정보가 추가된다.
