...
class PGPFeatureBuilder(AbstractFeatureBuilder):
    ...
    def get_features_from_simulation(
        ...
        return PGPFeatures(
            ...
            att_node_masks=agent_node_attention_masks, # PGPAgentNodeMasks{vehicle_node_masks ~ [노드 수, vehicle 수], 
                                                       #                   pedestrian_node_masks ~ [노드 수, vehicle 수]}
            traversal_features=traversal_features, # PGPTraversals{init_node ~ [노드 수, ],
                                                    #               node_seq_gt ~ [16<traversal horizon>, ],
                                                    #               visited_edges.shape ~ [노드 수, 최대 노드 이웃 수]}
        )
