...
class PGPEgoAgentsFeatureBuilder(AbstractFeatureBuilder):
    ...
    def get_features_from_simulation(
        ...
        indices = sample_indices_with_time_horizon(20, 2.0, history.sample_interval)

        try:
            sampled_past_observations = [cast(DetectionsTracks, past_observations[-idx]).tracked_objects for idx in reversed(indices)] # 특이하게 왜 1초까지의 데이터만 가져오는거지..?
    ...
    def _compute_feature(
        ...
        vehicle_tracks = filter_tracked_objects_by_type(past_and_current_detections, TrackedObjectType.VEHICLE)
        ...
        pedestrian_tracks = filter_tracked_objects_by_type(past_and_current_detections, TrackedObjectType.PEDESTRIAN)
        (pedestrians_agent_feats,pedestrians_agent_masks,) = self._get_surrounding_agents_representation(pedestrian_tracks, ego_states, time_stamps)
        ...
        return PGPEgoAgents(
            vehicle_agent_feats=vehicle_agent_feats, #(주변 차량의 수, 6, 5)
            vehicle_agent_masks=vehicle_agent_masks, # (주변 차량의 수, 6, 5)
            pedestrians_agent_feats=pedestrians_agent_feats, # (주변 행인의 수, 6, 5)
            pedestrians_agent_masks=pedestrians_agent_masks, # (주변 행인의 수, 6, 5)
            ego_feats=ego_feats, # (1, 6, 5)
        )
