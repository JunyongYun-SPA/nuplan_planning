...
class PGPGraphMapFeatureBuilder(AbstractFeatureBuilder):
    ...
    def get_features_from_simulation(
        ...
        return self._compute_feature(ego_state, map_api, route_roadblock_ids, traffic_light_status, current_input, initialization) # BH, current_input이랑 inintialization은 BH가 추가한거임
    ...
    def _compute_feature(self,
                        ...
                        current_input,
                        initialization,
                        correct_route_roadblock_ids: bool = True,) -> PGPGraphMap:

        # correct_route_roadblock_ids는 항상 True
        # 
        #
        if correct_route_roadblock_ids:
            route_roadblock_ids = get_correct_route_roadblock_ids(route_roadblock_ids=route_roadblock_ids,
                                                                ego_pose=ego_state.center,
                                                                map_api=map_api)
        
        # BH map visualalization 구축
        ...
        BH_vis_data_pack["current_iteration"] = current_input.iteration.index['index']
        BH_vis_data_pack["scenario_token"] = current_input.iteration.index['token']
        map_api = initialization.map_api
        position: Point2D = ego_state.center.point
        DRIVABLE_MAP_LAYERS = [SemanticMapLayer.ROADBLOCK,
                                SemanticMapLayer.ROADBLOCK_CONNECTOR,
                                SemanticMapLayer.CARPARK_AREA,
                                SemanticMapLayer.LANE, # BH
                                SemanticMapLayer.LANE_CONNECTOR]# BH
        Route_roadblock_id_seq = route_roadblock_ids
        Goal_loc = BH_vis_data_pack["goal_point"]
        ego_loc = BH_vis_data_pack["ego_location"]
        cur_iteration = BH_vis_data_pack["current_iteration"]
        current_scenario_token = BH_vis_data_pack["scenario_token"]
        drivable_area = map_api.get_proximal_map_objects(position, 500, DRIVABLE_MAP_LAYERS)
        if os.path.exists(f'/home/workspace/tmp_save_fig/{current_scenario_token }_fig.png') == False:
            #step1. Roadblock 가져오기
            Roadblocks = list(drivable_area.values())[0]
            Roadblock_ids = []
            Roadblock_polygons = []
            Roadblock_cons = list(drivable_area.values())[1]
            Roadblock_con_ids = []
            Roadblock_con_polygons = []
            Lanes = list(drivable_area.values())[3]
            Lane_ids = []
            Lane_polygons = []
            for Roadblock in Roadblocks:
                Roadblock_ids.append(Roadblock.id)
                Roadblock_polygons.append(Roadblock.polygon)
            for Roadblock_con in Roadblock_cons:
                Roadblock_con_ids.append(Roadblock_con.id)
                Roadblock_con_polygons.append(Roadblock_con.polygon)
            for Lane in Lanes:
                Lane_ids.append(Lane.id)
                Lane_polygons.append(Lane.polygon)
            ...
            figure_size_x = [Goal_loc[0]-out_boundary, Goal_loc[0]+out_boundary]
            figure_size_y = [Goal_loc[1]-out_boundary, Goal_loc[1]+out_boundary]
            ...
            plt.plot(Goal_loc[0], Goal_loc[1], marker='*', markersize=22, color='green')
            plt.plot(ego_loc[0], ego_loc[1], marker='s', markersize=16, color='Red')
            for Roadblock_polygon, id in zip(Roadblock_polygons, Roadblock_ids):
                x, y = Roadblock_polygon.exterior.xy
                if id in Route_roadblock_id_seq:
                    route_id_index = Route_roadblock_id_seq.index(id)
                    plt.fill(x, y, color='blue', edgecolor='blue')
                    #plt.text(np.mean(x), np.mean(y), str(route_id_index), fontsize=14, ha='right', va='bottom', bbox=mybox)
                else:
                    plt.fill(x, y, color='lightblue', edgecolor='blue')
            for con_polygon, con_id in zip(Roadblock_con_polygons, Roadblock_con_ids):
                x, y = con_polygon.exterior.xy
            ...
            for lane_polygon, lane_id in zip(Lane_polygons, Lane_ids):
                ...
                plt.fill(x, y, color='white', edgecolor='black', alpha=0.3)
                    
            plt.savefig(f'/home/workspace/tmp_save_fig/{current_scenario_token }_fig.png')
        ...
        return PGPGraphMap(
            lane_node_feats=lane_node_feats, # (노드 수, 각 노드를 구성하는 포인트 수, 7)
            lane_node_masks=lane_node_masks, # (노드 수, 각 노드를 구성하는 포인트 수, 7)
            lane_ids=lane_node_ids, # (노드 수, 1) 각 노드가 어떤 lane으로부터 분해된 것인지 알려주기 위함
            s_next=s_next, #(노드 수, 최대 이웃 수)
            edge_type=edge_type, # (노드 수, 최대 이웃 수)
            edge_on_route_mask=edge_on_route_mask, #(노드 수, 최대 이웃 수)
            nodes_on_route_flag=nodes_on_route_flag, #(노드 수, 각 노드를 구성하는 포인트 수, 1)
            red_light_flag=red_light_flag, #(노드 수, 각 노드를 구성하는 포인트 수, 1)
            red_light_mask=red_light_mask, #(노드 수, 최대 이웃 수)
        )
    ...
    def get_lane_node_feats(
        ...
        # BH 테스트용 visualization 코드
        # 검증할 내용: lane connector가 제대로 된 shape를 가지는가
        # lane_con_type = type(lanes[-1])
        # lane_type = type(lanes[0])
        # plt.figure(figsize=(22,22))
        # for lane in lanes:
        #     lane_color = 'red' if type(lane) == lane_type else 'blue'
        ...
        #         x.append(point.x)
        #         y.append(point.y)
        #     plt.scatter(x, y, color = lane_color, s=0.5, alpha=0.2)
        # plt.savefig("/home/workspace/tmp_save_fig/lane&con.png")
    ...
    def split_lanes(
        ...
        for idx, lane in enumerate(lanes):
            n_segments = int(np.ceil(len(lane) / max_len)) # 4 max len은 최대 세그먼트의 길이로 볼 수 있다.
            n_poses = int(np.ceil(len(lane) / n_segments)) # 16 n_poses는 segment의 실제 길이로 볼 수 있다.
