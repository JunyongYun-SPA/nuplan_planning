...
def build_lightning_datamodule(
    ...
    # Create data augmentation # PDM open 기준으로 data augmentation은 진행되지 않음(BH)
    ...
    #with open("/home/ssd/dataset/nuplan/cache/mini_set_token_list_ver3.pickle", 'wb') as f:
    #    pickle.dump(tokens, f, pickle.HIGHEST_PROTOCOL)

    # Create datamodule
    # BH가 Single worker로 실행되도록 수정함
    # cfg.data_loader.params['num_workers']=8
...
def build_trainer(cfg: DictConfig) -> pl.Trainer:
    ...
    if cfg.lightning.trainer.checkpoint.resume_training:
        ...
        OmegaConf.set_struct(cfg, False)
        last_checkpoint = cfg.checkpoint_path
