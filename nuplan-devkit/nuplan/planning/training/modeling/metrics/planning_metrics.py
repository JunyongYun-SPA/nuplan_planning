...
class AverageDisplacementError(AbstractTrainingMetric):
    ...
    def compute(self, predictions: TargetsType, targets: TargetsType, reg_mask) -> torch.Tensor:
        ...
        predicted_trajectory: Trajectory = Trajectory(predictions["trajectory"][:, :, :2]) #targets['pdm_features']['av_index']
        ...
        return (torch.norm(predicted_trajectory.xy - targets_trajectory.xy, p=2, dim=-1)).mean(dim=-1).mean()
...
class FinalDisplacementError(AbstractTrainingMetric):
    ...
    def compute(self, predictions: TargetsType, targets: TargetsType, reg_mask) -> torch.Tensor:
        ...
        predicted_trajectory: Trajectory = Trajectory(predictions["trajectory"][:, :, :2])
...
class FinalHeadingError(AbstractTrainingMetric):
    ...
    def compute(self, predictions: TargetsType, targets: TargetsType, reg_mask) -> torch.Tensor:
        ...
        try:
            predicted_trajectory = predictions["trajectory"][:, -1, 2]
