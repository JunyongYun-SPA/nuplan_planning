...
class LightningModuleWrapper(pl.LightningModule):
    ...
    def validation_step(
        ...
        occupancy_map_future = torch.zeros(16, 16, occupancy_size+1, occupancy_size+1, dtype=torch.float32).to(pi.device)
        ...
        av_mask = (edge_index[1].unsqueeze(1)==features['pdm_features'].av_index).any(dim=1) 
        ...
        batch_idx_for_agent = others_indx.unsqueeze(-1).repeat(1, 16) - features['pdm_features'].ptr.unsqueeze(0)[:, 1:]
        batch_idx_for_agent = torch.where((batch_idx_for_agent >= 0), 1, 0).sum(dim=-1)

        y_hat_av_centric = torch.bmm(y_hat[..., :2].reshape(-1, 16, 2).to(torch.float32), rotate_mat.repeat(y_hat.shape[0], 1, 1)).reshape(y_hat.shape[0], -1, 16, 2)
        ...
        # agent_pred = torch.where((((~features['pdm_features'].padding_mask[others_indx, 11:]).unsqueeze(0) * occ_mask_pred).unsqueeze(-1)),
        #                     agent_pred,
        ...
        # occupancy_y_pred[~((~features['pdm_features'].padding_mask[others_indx, 11:]).unsqueeze(0) * occ_mask_pred)] = \
        #     -1 * occupancy_y_pred[~((~features['pdm_features'].padding_mask[others_indx, 11:]).unsqueeze(0) * occ_mask_pred)]
        ...
        for time in range(16):
            occupancy_map_pred[:, time, :, :] = torch.where((occupancy_map_pred[:, time, :, :] >= 1), time+1, 0)
        
        with open("/home/workspace/validation_results/unimodal_ver8_v4/scene_list.pickle", 'rb') as fr:
            scene_list = pickle.load(fr)
            
        for idx, scene in enumerate(features['pdm_features'].seq_id):
            if scene in scene_list:
                pass
            else:
                continue
            fig1, axes1 = plt.subplots(1, 4, figsize=(40, 10))  # 3행 4열의 subplot 생성
            for k in range(1): 
                others_indx_2 = edge_index[0][np.where(edge_index[1].cpu() == features['pdm_features'].av_index[idx].cpu())[0]]
                agent_pred_2 = y_hat_av_centric[:, others_indx_2][:, :, :, :2] + features['pdm_features'].positions[others_indx_2][:, 10, :2].unsqueeze(0).unsqueeze(-2) #(1, 456, 16, 2)
                padding_mask = features['pdm_features'].padding_mask[others_indx_2]
                for i, ah in enumerate(agent_pred_2[k]):
                    axes1[0].plot(ah.cpu()[~padding_mask[i, 11:]][:, 0].detach().numpy(), ah.cpu()[~padding_mask[i, 11:]][:, 1].detach().numpy())
                    try:
                        axes1[0].scatter(ah.cpu()[~padding_mask[i, 11:]][-1, 0].detach().numpy(), ah.cpu()[~padding_mask[i, 11:]][-1, 1].detach().numpy())
                    except:
                        pass
                axes1[0].set_xlim([-50, 50])
                axes1[0].set_ylim([-50, 50])
        
            # occupancy_map_future = torch.where((occupancy_map_future >= 1), 1, 0)
            for j in range(16):
                axes1[1].imshow(occupancy_map_pred[idx, j, :200, :200].cpu().detach().numpy(), cmap='binary')  # 각 층의 occupancy map을 흑백 이미지로 표시 #.sum(dim=0) 
                axes1[1].set_title(f"Pred")  # subplot 제목 설정 
                axes1[2].imshow(predictions["occupancy_map"][idx, j, :200, :200].cpu().detach().numpy(), cmap='binary')  # 각 층의 occupancy map을 흑백 이미지로 표시 #.sum(dim=0) 
                axes1[2].set_title(f"Pred")  # subplot 제목 설정 
                plt.savefig(f'/home/workspace/validation_results/multimodal_unet_v4/vis/vis_{scene}_{j}.png') 
                plt.show()
            
            occupancy_map_future_total = occupancy_map_pred[idx, :, :200, :200].sum(dim=0)
            occupancy_map_future_total = torch.where((occupancy_map_future_total > 16), 16*torch.ones(occupancy_map_future_total.shape).to(occupancy_map_future_total.device).to(torch.float32), occupancy_map_future_total.to(torch.float32))
            axes1[3].imshow(occupancy_map_future_total.cpu().detach().numpy(), cmap='binary')  # 각 층의 occupancy map을 흑백 이미지로 표시 #.sum(dim=0) 
            axes1[3].set_title(f"Pred")  # subplot 제목 설정 
            plt.savefig(f'/home/workspace/validation_results/multimodal_unet_v4/vis/vis_{scene}_total.png') 
            plt.show()
