...
class DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        #     raise ValueError(f'Stage must be one of ["fit", "test"], got ${stage}.')
          ...
          # Testing Dataset
          test_samples = self._splitter.get_val_samples(self._all_samples, self._worker) # BH 수정
          assert len(test_samples) > 0, 'Splitter returned no test samples'
    ...
    def val_dataloader(self):
        ...
        return DataLoader(
            dataset=self._val_set, **self._dataloader_params,
        )
    ...
    def test_dataloader(self):
        ...
        return DataLoader(
            dataset=self._test_set, **self._dataloader_params,
        )
