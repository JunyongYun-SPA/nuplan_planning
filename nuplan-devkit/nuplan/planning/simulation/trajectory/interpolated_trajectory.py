...
class InterpolatedTrajectory(AbstractTrajectory):
    ...
    def __init__(self, trajectory: List[InterpolatableState]):
        ...
        self._trajectory = trajectory # 21개의 uncorrected와 12개의 corrected를 가져와서 길이 33의 ego state가 된다

        # Time points for interpolation axis
        time_series = [point.time_us for point in trajectory] # 각 ego state에서 time series를 가져온다
        ...
        for point in trajectory:
            ...
            '''
            linear_states.append(split_state.linear_states) # 길이 8
            angular_states.append(split_state.angular_states) # 길이 1
        ...
        self._function_interp_linear = sp_interp.interp1d(time_series, linear_states, axis=0) # interp 함수 생성
        self._angular_interpolator = AngularInterpolator(time_series, angular_states) # 동일하게 interp 함수 생성
