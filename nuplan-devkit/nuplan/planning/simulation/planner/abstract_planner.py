...
class AbstractPlanner(abc.ABC):
    ...
    def compute_trajectory(self, current_input: PlannerInput, tmp_scenario) -> AbstractTrajectory: # 원래 시리오는 절대 추가하면 안됨, BH
        """
        ...
        try:
            trajectory, forward_time = self.compute_planner_trajectory(current_input, tmp_scenario) 
        except Exception as e:
            self._compute_trajectory_runtimes.append(time.perf_counter() - start_time)
            raise e

        self._compute_trajectory_runtimes.append(forward_time)
        return trajectory
