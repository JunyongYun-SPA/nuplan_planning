...
class MLPlanner(AbstractPlanner):
    ...
    def compute_planner_trajectory(self, current_input: PlannerInput, tmp_scenario) -> AbstractTrajectory: # BH
        """
        Infer relative trajectory poses from model and convert to absolute agent states wrapped in a trajectory.
        Inherited, see superclass.
        """
        # # BH, Visulaization pack 추가
        ...
        # BH_vis_data_pack["current_iteration"] = current_input.iteration.index
        ...
        current_input.iteration.index = {'index':0, 'token':f'{tmp_scenario.scenario.token}'} #current_input에 시나리오 토큰을 숨겨서 전달함
