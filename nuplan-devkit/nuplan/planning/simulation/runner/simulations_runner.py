...
class SimulationRunner(AbstractRunner):
    ...
    def run(self) -> RunnerReport:
        ...
        sim_time_step = 0
        for _ in tqdm(range(self.simulation.scenario.get_number_of_iterations()), desc=f"{self.simulation.scenario.token} 시뮬레이션 중..."):# BH 시간이 보이도록 tqdm 추가
            # pass
            if self.simulation.is_simulation_running():
                # iteration, BH
                # print(f"BH: 현재 시뮬레이션 time step:{sim_time_step}")
                sim_time_step += 1
                # Execute specific callback
                ...
                trajectory = self.planner.compute_trajectory(planner_input, self.simulation.scenario) # 원래 시나리오는 추가하면 절대 안됨, BH
