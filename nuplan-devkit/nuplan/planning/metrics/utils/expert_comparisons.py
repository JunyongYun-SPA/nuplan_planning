...
def compute_traj_errors_v2(
    ...
    vector_displacements = np.zeros((traj_len-1, 2))
    ego_traj_vector = [[ego_traj[i+1].x - ego_traj[i].x, ego_traj[i+1].y - ego_traj[i].y] for i in range(traj_len-1)]
    expert_traj_vector = [[expert_traj[i+1].x - expert_traj[i].x, expert_traj[i+1].y - expert_traj[i].y] for i in range(expert_traj_len-1)]
    
    for i in range(traj_len-1):
        vector_displacements[i, :2] = [ego_traj_vector[i][0] - expert_traj_vector[i][0], ego_traj_vector[i][1] - expert_traj_vector[i][1]]
        # vector_displacements[i, 2] = ego_traj[i].heading - expert_traj[i].heading
        # vector_displacements[i, 2] = np.arctan2(np.sin(vector_displacements[i, 2]), np.cos(vector_displacements[i, 2]))
    
    dist_seq = np.hypot(vector_displacements[:, 0], vector_displacements[:, 1])
