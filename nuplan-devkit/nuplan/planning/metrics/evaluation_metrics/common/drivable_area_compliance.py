...
class DrivableAreaComplianceStatistics(MetricBase):
    ...
    def extract_metric(self, history: SimulationHistory) -> Tuple[List[float], bool]:
        ...
        return corners_in_drivable_area, far_from_drivable_area # 149개의 길이를 가지는 corners_in_da -> 모서리중 하나라도 나가면 0으로, far_from_da ->T/F
    ...
    def compute(self, history: SimulationHistory, scenario: AbstractScenario) -> List[MetricStatistics]:
        ...
        :return: the estimated metric.
        """
        corners_in_drivable_area, far_from_drivable_area = self.extract_metric(history=history) # 149개의 0또는 1로 구성된 리스트, 코너중 하나가 th이상으로 나갈 경우
