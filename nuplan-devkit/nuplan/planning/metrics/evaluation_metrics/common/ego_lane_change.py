...
class EgoLaneChangeStatistics(MetricBase):
    ...
    def compute(self, history: SimulationHistory, scenario: AbstractScenario) -> List[MetricStatistics]:
        ...
        ego_states = history.extract_ego_state # 길이 149의 state 리스트
        ego_poses = extract_ego_center(ego_states) # 길이 149의 point 리스트

        # Get the list of lane or lane_connectors associated to ego at each time instance, and store to use in other metrics
        self.ego_driven_route = get_route(history.map_api, ego_poses) # 149개의 ego 위치마다 해당되는 lane object를 뽑아서 리스트로 만든 것

        # Extract ego timepoints
        ego_timestamps = extract_ego_time_point(ego_states) # 149의 길이를 가지는 time stamp의 리스트

        # Extract corners of ego's footprint
        ego_footprint_list = [ego_state.car_footprint for ego_state in ego_states] # 149의 길이를 가지는 ego_footprint의 리스트

        # Extract corner lanes/lane connectors
        corners_route = extract_corners_route(history.map_api, ego_footprint_list) # 149개의 corner object를 가짐, 각 corner object에는 다음 4개의 정보가 담김(front_left_map_objs / front_right_map_objs / rear_left_map_objs / rear_right__map_objs)
