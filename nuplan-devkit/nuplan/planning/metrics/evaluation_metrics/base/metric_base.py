...
class MetricBase(AbstractMetricBuilder):
    ...
    def _construct_open_loop_metric_results(
        ...
        metric_statistics.extend(
            [
                Statistic(
                    ...
                    # value=np.mean(metric_values) <= maximum_threshold, # metric_values.shape ~(3, 15)
                    value = metric_values <= maximum_threshold, # metric_values.shape ~(3, 15)
        ...
        metric_values_over_horizons_at_each_time = metric_values
